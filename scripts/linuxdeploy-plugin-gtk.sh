...
# Defensive: refuse to proceed if APPDIR is not a directory or resolves to
# a filesystem root — writing to / would corrupt the host. This check runs
# BEFORE the upstream plugin to prevent any host filesystem mutation.
if [[ ! -d "$APPDIR" ]]; then
    echo "Error: --appdir ($APPDIR) is not a directory — refusing to touch host paths." >&2
    exit 1
fi
...
if [[ "$appdir_canonical" == "/" || "$appdir_canonical" == "." ]]; then
    echo "Error: --appdir resolves to a filesystem root ($appdir_canonical) — refusing to touch host paths." >&2
    exit 1
fi
...
# Fail-closed: if APPDIR is unset or usr/ is missing, the patched relative path
# would resolve against an arbitrary cwd, reproducing the original crash.
if [ -z "${APPDIR:-}" ]; then
    echo "Error: APPDIR is not set — cannot establish WebKitGTK cwd." >&2
    exit 1
fi
if [ ! -d "$APPDIR/usr" ]; then
    echo "Error: $APPDIR/usr does not exist — cannot establish WebKitGTK cwd." >&2
    exit 1
fi
...
case "$(uname -m)" in
    ...
    aarch64|arm64)  WEBKIT_ARCH_DIR="aarch64-linux-gnu" ;;
    *)
...
patch_webkit_libexec_path() {
    ...
    done < <(find "$APPDIR/usr" -xdev \( -type f -o -type l \) -name 'libwebkit2gtk-4.1.so*' -print0 2>/dev/null)
...
deploy_webkit_helpers || {
    echo "Error: WebKitGTK helper deployment failed — aborting packaging." >&2
    exit 1
}
patch_webkit_libexec_path || {
    echo "Error: WebKitGTK binary patch failed — aborting packaging." >&2
    exit 1
}
