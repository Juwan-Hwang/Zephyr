...
case "$(uname -m)" in
    ...
    *)
        echo "Error: Unsupported architecture $(uname -m) — WebKitGTK helper patch cannot be applied." >&2
        echo "  The AppImage would crash on non-Debian distributions." >&2
        exit 1
        ;;
esac
        ...
        for candidate in \
            ...
            if [[ -f "$candidate" ]]; then
                ...
                case "$lib" in
                    "$appdir_real"/*) ;;
                    *)
                        echo "Error: $candidate resolves outside AppDir ($lib) — refusing to patch." >&2
                        return 1
                        ;;
                esac
                break 2
