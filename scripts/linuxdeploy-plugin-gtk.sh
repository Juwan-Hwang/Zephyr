...
deploy_webkit_helpers() {
    ...
    # Verify all expected helpers are present.
    for expected in WebKitNetworkProcess WebKitWebProcess; do
        if [ ! -f "$target_dir/$expected" ]; then
            echo "Warning: $expected not found in $target_dir — WebKitGTK may crash." >&2
        fi
    done
...
patch_webkit_libexec_path() {
    ...
    if [ -z "$lib" ] || [ ! -f "$lib" ]; then
        echo "Warning: libwebkit2gtk-4.1.so not found in AppDir — skipping binary patch." >&2
        echo "  (This is expected for non-AppImage builds.)" >&2
        return 0
    fi
    ...
    if [ "$count" -eq 0 ]; then
        echo "Warning: PKGLIBEXECDIR string not found in $lib — WebKitGTK may use a different path." >&2
        echo "  Expected: $old" >&2
        return 0
    fi
