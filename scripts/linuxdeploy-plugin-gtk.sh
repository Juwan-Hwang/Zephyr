...
# Defensive: refuse to proceed if APPDIR is unset, not a directory, or
# is a filesystem root — writing to / would corrupt the host.
# The top-level arg parser already checks this, but an explicit guard
# here protects against sourcing the script or calling functions in
# isolation, and protects all subsequent filesystem writes (hook
# creation, helper bundling, binary patching).
appdir_canonical="$(readlink -f "$APPDIR")" || {
    echo "Error: failed to resolve APPDIR real path ($APPDIR)." >&2
    exit 1
}
if [[ -z "$APPDIR" || ! -d "$APPDIR" || "$appdir_canonical" == "/" || "$appdir_canonical" == "." ]]; then
    echo "Error: --appdir is missing, not a directory, or resolves to a filesystem root — refusing to touch host paths." >&2
    exit 1
fi
