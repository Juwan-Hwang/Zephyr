...
fn split_ambient_proxy_segments(raw: &str) -> Vec<&str> {
    ...
    for (i, &b) in raw.as_bytes().iter().enumerate() {
        if b == b'\n' || b == b'\r' {
            ...
            let has_scheme_or_tag = remainder
                .find(|c: char| !c.is_ascii_alphanumeric() && c != '+' && c != '-' && c != '.')
                .is_some_and(|idx| {
                    let tag = &remainder[..idx];
                    let rest = &remainder[idx..];
                    !tag.is_empty() && (rest.starts_with('=') || rest.starts_with("://"))
                });
...
pub fn matches_no_proxy_rules_with_port(host: &str, port: Option<u16>, no_proxy_val: &str) -> bool {
    ...
    for raw_pat in no_proxy_val.split(|c: char| c == ',' || c == ';' || c.is_ascii_whitespace()) {
        ...
        if pat_host.eq_ignore_ascii_case("<local>") && !trimmed_host.contains('.') {
            return true;
        }
        ...
        let domain_suffix = pat_host
            ...
            .unwrap_or(pat_host);
        if trimmed_host.eq_ignore_ascii_case(domain_suffix) {
            return true;
        }
