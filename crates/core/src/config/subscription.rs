...
fn parse_ambient_proxy_segment(segment: &str) -> Option<AmbientCandidate> {
    ...
    // Strip extraneous "http://" or "https://" prefix if followed by a scheme prefix in the authority (e.g. "http://http=127.0.0.1:7890")
    ...
    // Handle potential scheme prefixes like "http=host:port", "https=host:port", "socks=...", "socks5=..."
    // Scheme-prefix format applies to the authority segment (e.g. "http=127.0.0.1:7890"), not URLs where '=' only appears in path/query.
    let authority = seg.split('/').next().unwrap_or(seg);
    let (tag, default_scheme, inner) = if let Some((prefix, rest)) = authority.split_once('=') {
        let p_lower = prefix.trim().to_ascii_lowercase();
        let remainder = if seg.len() > authority.len() {
            &seg[prefix.len() + 1..]
        } else {
            rest.trim()
        };
        if matches!(p_lower.as_str(), "socks" | "socks5") {
            (Some("socks".to_owned()), "socks5", remainder)
        } else if p_lower == "socks5h" {
            (Some("socks".to_owned()), "socks5h", remainder)
        } else if p_lower == "socks4" {
            (Some("socks".to_owned()), "socks4", remainder)
        } else if p_lower == "socks4a" {
            (Some("socks".to_owned()), "socks4a", remainder)
        } else if matches!(p_lower.as_str(), "https" | "http" | "all") {
            (Some(p_lower), "http", remainder)
        } else {
            // Unknown protocol key (e.g. "ftp="): not usable as an HTTP/SOCKS proxy.
            return None;
        }
    } else {
        (None, "http", seg)
...
pub fn collect_ambient_proxy_urls_for_scheme(
    ...
    let mut push_dedup = |u: &str| {
        if !result.iter().any(|existing: &String| existing == u) {
