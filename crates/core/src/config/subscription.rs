...
/// Check if a host matches a comma-separated `NO_PROXY` rule list.
#[must_use]
pub fn matches_no_proxy_rules(host: &str, no_proxy_val: &str) -> bool {
    ...
    for raw_pat in no_proxy_val.split(',') {
        let pat = raw_pat.trim().trim_matches('[').trim_matches(']');
        if pat.is_empty() {
            continue;
        }
        if pat == "*" {
        ...
        if trimmed_host.eq_ignore_ascii_case(pat) {
            return true;
        }
        let domain_suffix = pat.strip_prefix('.').unwrap_or(pat);
...
pub fn is_host_in_no_proxy(host: &str) -> bool {
    let val = std::env::var("NO_PROXY")
        ...
        .unwrap_or_default();
    matches_no_proxy_rules(host, &val)
}
