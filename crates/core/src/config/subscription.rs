...
/// Helper to determine if a host string represents a loopback address (IPv4, IPv6, or localhost).
#[must_use]
...
fn is_ipv4_loopback_host(h: &str) -> bool {
    let unbracketed = h.trim_start_matches('[').trim_end_matches(']');
    unbracketed.eq_ignore_ascii_case("localhost") || unbracketed == "127.0.0.1"
}

fn is_ipv6_loopback_host(h: &str) -> bool {
    let unbracketed = h.trim_start_matches('[').trim_end_matches(']');
    unbracketed == "::1"
}

const fn proxy_scheme_family(scheme: &str) -> &'static str {
    if scheme.eq_ignore_ascii_case("http") || scheme.eq_ignore_ascii_case("https") {
        ...
        || scheme.eq_ignore_ascii_case("socks4")
        || scheme.eq_ignore_ascii_case("socks4a")
    {
        "socks"
    } else {
        "other"
...
/// Checks whether two proxy candidate URLs target the same loopback endpoint (same port on a loopback host with compatible scheme family).
#[must_use]
pub fn is_same_loopback_proxy_endpoint(amb: &str, mihomo: &str) -> bool {
    if amb == mihomo {
        return true;
    }
    if let (Ok(u1), Ok(u2)) = (url::Url::parse(amb), url::Url::parse(mihomo)) {
        if proxy_scheme_family(u1.scheme()) != proxy_scheme_family(u2.scheme()) {
            return false;
        }
        ...
        if p1 == p2 && p1.is_some() {
            ...
            if is_ipv4_loopback_host(h1) && is_ipv4_loopback_host(h2) {
                return true;
            }
            if is_ipv6_loopback_host(h1) && is_ipv6_loopback_host(h2) {
                return true;
            }
        }
    }
...
fn map_protocol_tag(prefix: &str) -> Option<(Option<String>, &'static str)> {
    ...
    match p_lower.as_str() {
        "socks" | "socks5" | "socks5h" => Some((Some("socks".to_owned()), "socks5h")),
...
fn parse_ambient_proxy_segment(segment: &str) -> Option<AmbientCandidate> {
    ...
    if let Some(prefix_len) = wrapper_len {
        let rest = &seg[prefix_len..];
    ...
    let candidate = if inner
        .get(..8)
        .is_some_and(|p| p.eq_ignore_ascii_case("socks://"))
    {
        format!("socks5h://{}", &inner[8..])
    } else if inner
        .get(..9)
        .is_some_and(|p| p.eq_ignore_ascii_case("socks5://"))
    {
        format!("socks5h://{}", &inner[9..])
            ...
            let is_separator = if !is_proxy_entry_start(remainder) {
                ...
                let in_userinfo = if let Some(raw_part) = raw.get(start..i) {
                    if let Some(slash_idx) = raw_part.find("://") {
                        ...
                        if url::Url::parse(raw_part.trim()).is_ok() || remainder.contains("://") {
                            false
                        } else if !after_scheme.contains('@') && !after_scheme.contains('/') {
                            let delims = [
                                remainder.find(';'),
                                remainder.find(' '),
                                remainder.find('\n'),
...
pub fn collect_ambient_proxy_urls_for_scheme(
    ...
    if let Some(target) = target_scheme {
        ...
        // 5. For HTTPS target, allow HTTP-tagged proxy candidates as fallback
        // (Standard HTTP forward proxies support HTTPS tunneling via CONNECT method)
        if t_lower == "https" {
            for c in candidates
                .iter()
                .filter(|c| c.tag.as_deref() == Some("http"))
            {
                push_dedup(&c.url);
            }
        }
    } else {
        // 6. If no target scheme was specified, add all candidates in discovery order
        for c in &candidates {
            push_dedup(&c.url);
        }
    }
    ...
    let resolves_to_effective_proxy = |start_name: &str| -> bool {
        ...
        for _ in 0..max_depth {
            ...
            if let Some(p_obj) = proxies.get(curr) {
                ...
                return p_obj.get("alive").and_then(serde_json::Value::as_bool) != Some(false);
            }
            return false;
        }
        false
...
mod tests {
    ...
    fn test_validate_ambient_proxy_url() {
        ...
        assert_eq!(
            ...
            Some("socks5h://user:pass@127.0.0.1:1080".to_owned())
        );
    ...
    fn test_validate_ambient_proxy_url_credentials_with_equals() {
        assert_eq!(
            ...
            Some("socks5h://user:p=ss@127.0.0.1:1080".to_owned())
        );
        assert_eq!(
            validate_ambient_proxy_url("socks5://user:p=ss=word@127.0.0.1:1080"),
            Some("socks5h://user:p=ss=word@127.0.0.1:1080".to_owned())
        );
        assert_eq!(
    ...
    fn test_collect_ambient_proxy_urls_with_path_in_multi_entry() {
        ...
        assert_eq!(
            candidates,
            vec!["http://127.0.0.1:7890/sub_proxy".to_owned()]
        );
    }

    #[test]
    fn test_validate_ambient_proxy_url_uppercase_socks() {
        assert_eq!(
            validate_ambient_proxy_url("SOCKS://127.0.0.1:1080"),
            Some("socks5h://127.0.0.1:1080".to_owned())
        );
    ...
    fn test_split_ambient_proxy_segments_with_socks5h() {
        ...
        let raw_tags = "http=127.0.0.1:7890 socks=127.0.0.1:1080";
        assert_eq!(
            split_ambient_proxy_segments(raw_tags),
            vec!["http=127.0.0.1:7890", "socks=127.0.0.1:1080"]
        );
    }
    ...
    fn test_split_ambient_proxy_segments_preserves_url_semicolons() {
        ...
        // Semicolon between scheme URL and tag where second tag has credentials
        assert_eq!(
            split_ambient_proxy_segments("http://127.0.0.1:7890;https=user:pass@127.0.0.1:8443"),
            vec!["http://127.0.0.1:7890", "https=user:pass@127.0.0.1:8443"]
        );
        assert_eq!(
            split_ambient_proxy_segments("https://127.0.0.1:7890;https=user:pass@127.0.0.1:8443"),
            vec!["https://127.0.0.1:7890", "https=user:pass@127.0.0.1:8443"]
        );
    ...
    #[test]
    fn test_parse_ambient_proxy_segment_utf8_safety() {
        assert_eq!(validate_ambient_proxy_url("socksü€"), None);
        assert_eq!(validate_ambient_proxy_url("http=socksü€"), None);
        assert_eq!(
            validate_ambient_proxy_url("socks://127.0.0.1:1080"),
            Some("socks5h://127.0.0.1:1080".to_owned())
        );
        ...
        assert_eq!(
            validate_ambient_proxy_url("HTTPS://socks=127.0.0.1:1080"),
            Some("socks5h://127.0.0.1:1080".to_owned())
        );
    }

    #[test]
    fn test_split_ambient_proxy_segments_with_tab() {
        let raw = "http=127.0.0.1:7890\tsocks=127.0.0.1:1080";
        assert_eq!(
            split_ambient_proxy_segments(raw),
            vec!["http=127.0.0.1:7890", "socks=127.0.0.1:1080"]
        );
    }
    ...
    fn test_matches_no_proxy_rules_ports_and_cidr() {
        ...
        // Bare IPv6 rules without brackets
        assert!(matches_no_proxy_rules_with_port("::1", Some(80), "::1"));
        assert!(matches_no_proxy_rules_with_port(
            "2001:db8::1",
            Some(443),
            "2001:db8::1"
    ...
    #[test]
    fn test_is_same_loopback_proxy_endpoint() {
        // Exact match
        assert!(is_same_loopback_proxy_endpoint(
            "http://127.0.0.1:7890",
            "http://127.0.0.1:7890"
        ));
        // IPv6 bracketed vs IPv4 loopback are different address families
        assert!(!is_same_loopback_proxy_endpoint(
            "http://[::1]:7890",
            "http://127.0.0.1:7890"
        ));
        ...
        // localhost vs IPv4 loopback
        assert!(is_same_loopback_proxy_endpoint(
            "http://localhost:7890",
            "http://127.0.0.1:7890"
        ));
        // socks5 vs http scheme on loopback are different protocol families
        assert!(!is_same_loopback_proxy_endpoint(
            "socks5://[::1]:7890",
            "http://127.0.0.1:7890"
        ));
        ...
        // Equivalent SOCKS schemes on same loopback
        assert!(is_same_loopback_proxy_endpoint(
            "socks5h://127.0.0.1:7890",
            "socks5://127.0.0.1:7890"
        ));
        // 127.0.0.2 distinct loopback address not treated as duplicate of 127.0.0.1
        assert!(!is_same_loopback_proxy_endpoint(
            "http://127.0.0.2:7890",
            "http://127.0.0.1:7890"
        ));
        // Same distinct loopback address
        assert!(is_same_loopback_proxy_endpoint(
            "http://127.0.0.2:7890",
            "http://127.0.0.2:7890"
        ));
    ...
    #[test]
    fn test_is_single_label_host() {
        assert!(is_single_label_host("localhost"));
        assert!(is_single_label_host("router"));
        assert!(is_single_label_host("router."));
        assert!(!is_single_label_host("example.com"));
        assert!(!is_single_label_host("example.com."));
        assert!(!is_single_label_host("127.0.0.1"));
        assert!(!is_single_label_host("::1"));
        assert!(!is_single_label_host("[::1]"));
        assert!(!is_single_label_host("192.168.1.1"));
    }

    #[test]
    fn test_split_ambient_proxy_segments_semicolon_in_credentials_with_tag() {
        let raw = "https://user:p;http=x@127.0.0.1:8443;http=127.0.0.1:7890";
        let segments = split_ambient_proxy_segments(raw);
        assert_eq!(
            segments,
            vec![
                "https://user:p;http=x@127.0.0.1:8443",
                "http=127.0.0.1:7890"
            ]
        );
    }
