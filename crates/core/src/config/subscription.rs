...
fn is_standard_loopback_host(h: &str) -> bool {
    let unbracketed = h.trim_start_matches('[').trim_end_matches(']');
...
/// Checks whether two proxy candidate URLs target the same loopback endpoint (same port on a loopback host).
#[must_use]
pub fn is_same_loopback_proxy_endpoint(amb: &str, mihomo: &str) -> bool {
    ...
    if let (Ok(u1), Ok(u2)) = (url::Url::parse(amb), url::Url::parse(mihomo)) {
        ...
        if p1 == p2 && p1.is_some() {
            ...
            if is_standard_loopback_host(h1) && is_standard_loopback_host(h2) {
                return true;
            }
        }
    }
            ...
            let is_separator = if !is_proxy_entry_start(remainder) {
                ...
                let in_userinfo = if let Some(raw_part) = raw.get(start..i) {
                    if let Some(slash_idx) = raw_part.find("://") {
                        ...
                        if url::Url::parse(raw_part.trim()).is_ok()
                            || parse_assignment_tag(remainder).is_some()
                            || remainder.contains("://")
                        {
                            false
...
mod tests {
    ...
    fn test_is_same_loopback_proxy_endpoint() {
        ...
        // IPv6 bracketed vs IPv4 loopback with same port
        assert!(is_same_loopback_proxy_endpoint(
            "http://[::1]:7890",
            "http://127.0.0.1:7890"
        ));
        // localhost vs IPv6 loopback
        assert!(is_same_loopback_proxy_endpoint(
            "http://localhost:7890",
            "http://[::1]:7890"
        ));
        // socks5 vs http scheme on loopback with same port
        assert!(is_same_loopback_proxy_endpoint(
            "socks5://[::1]:7890",
            "http://127.0.0.1:7890"
        ));
