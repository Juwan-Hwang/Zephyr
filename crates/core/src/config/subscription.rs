    ...
    for (i, &b) in raw.as_bytes().iter().enumerate() {
        if b == b'\n' || b == b'\r' {
            ...
            let is_separator = if b == b' ' {
                !remainder.is_empty()
                    && (remainder.starts_with("http=")
                        || remainder.starts_with("https=")
                        || remainder.starts_with("socks=")
                        || remainder.starts_with("socks4=")
                        || remainder.starts_with("socks5=")
                        || remainder.starts_with("http://")
                        || remainder.starts_with("https://")
                        || remainder.starts_with("socks://")
                        || remainder.starts_with("socks4://")
...
pub fn collect_ambient_proxy_urls_for_scheme(
    ...
    let candidates: Vec<AmbientCandidate> =
        if let Some(single) = parse_ambient_proxy_segment(trimmed) {
            vec![single]
        } else {
            split_ambient_proxy_segments(trimmed)
                .into_iter()
