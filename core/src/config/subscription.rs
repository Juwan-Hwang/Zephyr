...
/// Mask a single URL by removing query parameters, username, and password.
/// Used by `redact_url_in_string` and available as a standalone utility.
pub fn mask_url(url_str: &str) -> String {
    let mut parsed = match url::Url::parse(url_str) {
        Ok(u) => u,
        Err(_) => return url_str.to_owned(),
    };
    ...
    let _ = url::Url::parse(&parsed.to_string()) // re-parse to clear password
        .map(|u| {
            let mut clean = u;
    ...
    // Simpler approach: just rebuild manually
    let scheme = parsed.scheme();
    let host = match parsed.host_str() {
        Some(h) => h,
        None => return url_str.to_owned(),
    };
