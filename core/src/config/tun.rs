...
pub fn update_tun_in_yaml(content: &str, enable: bool) -> Result<String, AppError> {
    ...
    let tun = mapping
        ...
        .or_insert_with(|| YamlValue::Mapping(yaml::Mapping::new()));
...
/// the fd using serde_yaml (not regex), and disables `auto-route` and
/// `auto-detect-interface` since VPN Service already handles routing.
#[cfg_attr(feature = "uniffi", uniffi::export)]
