...
5. **Commit changes** with a clear, descriptive message:
   ...
   **CRITICAL: Only list REAL fixes in the commit message.** Every item in the commit message must correspond to an actual change in the diff against `main`. Before writing the commit message, verify each claimed fix by checking `git diff main -- <file>`. Do NOT fabricate or exaggerate fixes — if a change is minor (e.g. a variable rename), describe it accurately rather than inflating its significance.
