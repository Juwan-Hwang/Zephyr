...
async fn get_expected_sha256(version: &str, asset_name: &str) -> Result<String, String> {
    ...
    Err(format!(
        "Could not find SHA256 hash for {}. Verification is required for security.",
        asset_name
    ))
}
...
pub async fn update_core(
    ...
    let archive_path = paths
        .core_dir
        .join(format!("core_update_{}.tmp", temp_suffix));
...
pub async fn update_geo_data(window: Window) -> Result<String, String> {
    ...
    let geoip_path = paths
        .core_dir
        .join(format!("geoip_{}.dat.tmp", temp_suffix));
    let response = client
        .get(geoip_url)
        .send()
        .await
    ...
    let geosite_path = paths
        .core_dir
        .join(format!("geosite_{}.dat.tmp", temp_suffix));
    let response = client
        .get(geosite_url)
        .send()
        .await
