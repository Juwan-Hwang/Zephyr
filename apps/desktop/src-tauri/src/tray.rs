...
fn copy_proxy_env_to_clipboard(app: &AppHandle) -> Result<(), String> {
    ...
    let port = mihomo_state
        ...
        .lock()
        .ok()
        .and_then(|g| g.last_proxy_port())
        .unwrap_or(DEFAULT_MIXED_PORT);

    let proxy = format!("http://127.0.0.1:{port}");
    let text = match format.as_str() {
        "fish" => format!(
            "set -x http_proxy {proxy}; set -x https_proxy {proxy}; set -x all_proxy {proxy}"
        ),
        "cmd" => format!(
...
fn handle_menu_event(app: &AppHandle, id: &str) {
    match id {
        ...
        "copy_env" => {
            if let Err(e) = copy_proxy_env_to_clipboard(app) {
                eprintln!("[tray] Failed to copy proxy env: {e}");
                crate::backend_event::emit_to_main(app, "tray-copy-env-failed", e);
            }
        }
        _ => {
