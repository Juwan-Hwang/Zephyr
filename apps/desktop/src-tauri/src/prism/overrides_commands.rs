...
pub fn override_toggle(state: State<PrismState>, id: String, enabled: bool) -> Result<(), String> {
    ...
    // When disabling a Prism YAML override, remove its active patch file
    // so the Prism extension stops applying it
    if !enabled {
        if let Ok(item) = overrides_store::load_item(&state, &id) {
            if item.ext == OverrideExt::PrismYaml {
                if let Ok(prism_dir) = crate::prism::prism_data_dir(&state.app) {
                    let _ = std::fs::remove_file(prism_dir.join(item.patch_filename()));
                }
            }
        }
    }
...
pub async fn override_apply_all(state: State<'_, PrismState>) -> Result<Vec<OverrideLog>, String> {
    ...
    let current_profile_id: Option<String> = {
        ...
        let lock = mihomo_state.0.lock().ok();
        lock.and_then(|l| l.last_config_path().map(String::from))
    };
