...
fn detect_override_ext(content: &str) -> OverrideExt {
    ...
    for line in content.lines().take(1000) {
        ...
        // Track block comments — skip their content entirely
        if in_block_comment {
            if let Some(end_pos) = trimmed.find("*/") {
                in_block_comment = false;
                // Process the remainder of the line after `*/`
                let remainder = trimmed[end_pos + 2..].trim();
                if !remainder.is_empty() {
                    // Re-analyze the remainder as a normal line below
                    // by not continuing — but we need a separate pass.
                    // For simplicity, just count semicolons and check length.
                    semicolons = semicolons.saturating_add(
                        u32::try_from(remainder.chars().filter(|&c| c == ';').count())
                            .unwrap_or(u32::MAX),
                    );
        ...
        if trimmed.starts_with("/*") {
            if let Some(end_pos) = trimmed.find("*/") {
                // Single-line block comment: `/* ... */ code...`
                // Process the remainder after `*/`
                let remainder = trimmed[end_pos + 2..].trim();
                if !remainder.is_empty() {
                    semicolons = semicolons.saturating_add(
                        u32::try_from(remainder.chars().filter(|&c| c == ';').count())
                    ...
                    if remainder.starts_with("\"use strict\"")
                        || remainder.starts_with("'use strict'")
                    {
                        js_signals += 3;
                    }
                }
        ...
        // Count semicolons (strong minified-JS indicator)
        semicolons = semicolons.saturating_add(
