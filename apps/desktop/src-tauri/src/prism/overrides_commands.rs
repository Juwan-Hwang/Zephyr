...
pub async fn override_refresh_remote(
    ...
    // Guard against memory exhaustion: read body as a byte stream with hard byte limit.
    // bytes_stream() yields chunks; we accumulate them while checking total byte count.
    // If total exceeds MAX_REMOTE_SIZE, we return an error immediately without draining
    // the stream — dropping the response/stream closes the underlying connection.
    ...
    while let Some(chunk_result) = stream.next().await {
        ...
        if buf.len() + chunk.len() > MAX_REMOTE_SIZE {
            // Return error immediately; dropping stream closes the connection
            return Err(format!(
                "Remote override content exceeds {MAX_REMOTE_SIZE} bytes"
            ));
        }
    ...
    // Input size validation (covers edge cases)
...
pub async fn override_apply_all(state: State<'_, PrismState>) -> Result<Vec<OverrideLog>, String> {
    ...
    for item in meta.items.iter().filter(|i| i.enabled) {
        ...
        // Propagate read errors instead of silently defaulting to empty
        let content = match overrides_store::read_content(&state, &item.id) {
            Ok(c) if !c.is_empty() => c,
            Ok(_) => {
                // Empty content: log a failure entry for this override
                let log = OverrideLog {
                    script_id: item.id.clone(),
                    script_name: item.name.clone(),
                    executed_at: chrono::Utc::now().timestamp_millis(),
            ...
            Err(e) => {
                // Read error: log a failure entry for this override
                let log = OverrideLog {
                    script_id: item.id.clone(),
                    script_name: item.name.clone(),
                    executed_at: chrono::Utc::now().timestamp_millis(),
