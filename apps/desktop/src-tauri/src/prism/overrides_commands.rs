...
pub fn override_toggle(state: State<PrismState>, id: String, enabled: bool) -> Result<(), String> {
    ...
    if item.ext == OverrideExt::PrismYaml {
        let Ok(prism_dir) = crate::prism::prism_data_dir(&state.app) else {
            return Ok(());
        };
        ...
        if enabled {
            ...
            if let Ok(content) = overrides_store::read_content(&state, &id) {
                let normalized = crate::prism::types::normalize_to_prism_yaml(&content);
                let _ = write_file_secure(&patch_path, &normalized);
            }
        } else {
