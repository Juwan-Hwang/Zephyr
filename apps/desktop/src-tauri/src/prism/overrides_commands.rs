...
pub async fn override_refresh_remote(
    ...
    // Each chunk is checked before accumulation; if total exceeds MAX_REMOTE_SIZE,
    // the stream is drained and an error is returned.
    // The network socket may still buffer excess bytes, but our process memory is capped.
    ...
    while let Some(chunk_result) = stream.next().await {
        ...
        if buf.len() + chunk.len() > MAX_REMOTE_SIZE {
            // Drain remaining stream to prevent socket buffering
