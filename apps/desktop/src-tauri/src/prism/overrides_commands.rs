...
/// Tracks block comments (`/* */`) and recognises minified JS patterns.
fn detect_override_ext(content: &str) -> OverrideExt {
    ...
    for line in content.lines().take(1000) {
        // Skip line comments before block-comment stripping to avoid
        // false `/*` detection inside `// ...` or `# ...` comments.
        if !in_block_comment {
            let pre_trimmed = line.trim();
            if pre_trimmed.starts_with("//") || pre_trimmed.starts_with('#') {
                continue;
            }
        }

        let stripped = strip_block_comments(line, &mut in_block_comment);
        let trimmed = stripped.as_ref().trim();
...
/// Strip block comments from a line, tracking multi-line comment state.
///
/// Returns a `Cow` to avoid allocation when no block comments are present.
/// The `in_block_comment` flag persists across calls to handle multi-line comments.
fn strip_block_comments<'a>(
    line: &'a str,
    in_block_comment: &mut bool,
) -> std::borrow::Cow<'a, str> {
    // Fast path: no block comment markers and not inside a block comment
