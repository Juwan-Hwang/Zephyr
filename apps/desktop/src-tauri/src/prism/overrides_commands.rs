...
pub async fn override_refresh_remote(
    ...
    // Guard against memory exhaustion: check Content-Length before reading body
    const MAX_REMOTE_SIZE: u64 = 10 * 1024 * 1024; // 10 MB limit
    if let Some(len) = response.content_length() {
        if len > MAX_REMOTE_SIZE {
            return Err(format!(
                "Remote override content too large: {len} bytes (max {MAX_REMOTE_SIZE})"
            ));
        }
    }
    ...
    // Double-check size after reading (in case Content-Length was missing or lied)
