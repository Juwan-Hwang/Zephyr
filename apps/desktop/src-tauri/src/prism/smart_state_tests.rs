...
mod tests {
    ...
    mod smart_state_tests {
        ...
        async fn record_single() {
            ...
            let state = SmartState::init(config)
                .await
    ...
    mod persistence_tests {
        ...
        async fn data_persists_after_threshold_flush() {
            ...
            for i in 0..12 {
                state
                    .record("node", i as f64 * 10.0, true)
            ...
            let config = SmartStateConfig::new(temp_dir.path());
            let state2 = SmartState::init(config)
                .await
                .expect("Failed to init state");
        ...
        async fn rapid_node_switching_triggers_flush() {
            ...
            for i in 0..100 {
                let node = nodes.get(i % nodes.len()).copied().unwrap_or("a");
                let result = state.record(node, i as f64, true);
                assert!(result.is_ok());
            }
    ...
    mod stress_tests {
        ...
        async fn many_nodes_stress() {
            ...
            for node_i in 0..1000 {
                let history = state
                    .get_history(&format!("node{node_i}"))
