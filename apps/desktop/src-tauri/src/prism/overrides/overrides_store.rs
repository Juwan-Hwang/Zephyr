...
/// Reorder override items by moving each item to the position specified in `ids`.
pub fn reorder_items(meta: &mut OverrideMeta, ids: &[String]) -> Result<(), String> {
    let _guard = acquire_meta_lock()?;

    // O(N) approach: build a HashMap for fast lookups
    // Use std::mem::take to temporarily move items out of meta
    ...
    #[allow(clippy::cast_possible_truncation)]
    let remaining_order = new_items.len() as u32;
    #[allow(clippy::iter_over_hash_type)]
    for (_, mut item) in items_map {
        item.order = remaining_order;
        new_items.push(item);
    }
