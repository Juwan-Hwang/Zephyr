...
/// 将 JSON 中的 null 值替换为 0（兼容旧版本数据中的 null 数值字段）
...
/// 变更记录 (WAL 格式)
///
/// timestamp 字段用于去重：主文件刷新后，WAL 中该 timestamp 之前的记录都视为已处理
...
impl SmartState {
    ...
    async fn recover(config: &SmartStateConfig) -> Result<DashMap<String, NodeHistory>, String> {
        ...
        let main_file_timestamp = if config.data_path.exists() {
            ...
            // 先尝试直接解析，失败则清洗 null 值后重试（兼容旧版本数据）
            let loaded: BTreeMap<String, NodeHistory> = if let Ok(map) = serde_json::from_str(&data)
            {
                map
            } else {
                eprintln!("[smart_state] smart_history.json has null values, sanitizing...");
            ...
            while let Ok(Some(line)) = lines.next_line().await {
                ...
                if let Ok(record) = serde_json::from_str::<ChangeRecord>(&line) {
                    ...
                    if let Some(cutoff_ts) = main_file_timestamp {
                        let ChangeRecord::Update { timestamp, .. } = &record;
    ...
    async fn writer_task(
        ...
        while let Some(record) = rx.recv().await {
            ...
            // 2. 阈值触发刷新（避免节点切换时频繁刷盘）
            if buffer.len() >= config.flush_threshold {
                if Self::flush_main(&histories, &config.data_path)
                    .await
                    .is_ok()
                {
