...
pub async fn prism_rebuild(
    ...
    let (raw_profile, current_secret, global_prefs) = {
        ...
        let prefs = core_process::GlobalPreferences {
            mode: settings_lock.mode.clone(),
            tun_enabled: settings_lock.tun_enabled,
            mixed_port: settings_lock.mixed_port,
            socks_port: settings_lock.socks_port,
            http_port: settings_lock.http_port,
            ipv6: settings_lock.ipv6,
            allow_lan: settings_lock.allow_lan,
            unified_delay: settings_lock.unified_delay,
        };
        drop(settings_lock);
