...
static RUN_CONFIG_LOCK: Mutex<()> = Mutex::new(());
...
fn build_script_context() -> ScriptContext {
    ScriptContext {
...
pub fn execute_batch_pipeline(
    ...
    for (script, name) in scripts {
        let result = runtime.execute_with_write(script, name, &config);
        if result.success && result.config_modified {
            if let Some(modified) = &result.modified_config {
                config = modified.clone();
                any_modified = true;
            }
        }
