        ...
        if let Some(lvl) = &level {
            ...
            if lvl_lower.is_empty() || lvl_lower == "all" {
                    ...
                    > 0
                {
                    ...
                    // Limit search to the line prefix (first 60 bytes) to avoid
                    // false positives when severity keywords appear in message body.
                    lower_buf.clear();
                    lower_buf.extend(buf.iter().map(u8::to_ascii_lowercase));
                    let search_limit = std::cmp::min(lower_buf.len(), 60);
                    let search_buf = lower_buf.get(..search_limit).unwrap_or(&lower_buf);
