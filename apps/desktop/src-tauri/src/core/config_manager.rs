...
fn cleanup_dangling_last_config(
    ...
    let needs_cleanup = {
        ...
        // 规范化后比较，避免 last_config 存储时缺少扩展名（如 "foo" vs "foo.yaml"）导致漏判
        // 同时 strip 掉 .yaml/.yml 扩展名后再比较，避免 "foo.yaml" vs "foo.yml" 漏判
        settings_guard.last_config.as_deref().is_some_and(|lc| {
            if lc == deleted_name {
                return true;
            }
            let strip_ext = |name: &str| -> String {
            ...
            !norm_lc.is_empty() && strip_ext(&norm_lc) == strip_ext(&norm_del)
        })
    };
    ...
    // 第二段持锁：只更新 last_config 并 clone 出快照
    let settings_clone = {
