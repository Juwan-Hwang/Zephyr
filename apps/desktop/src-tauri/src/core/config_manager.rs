...
pub fn export_logs(
    ...
    for (relative_path, abs_path) in &files_to_export {
        ...
        file.read_to_string(&mut content)
            .map_err(|e| format!("Failed to read log file {abs_path:?}: {e}"))?;

        // Filter by severity level if specified
        let filtered_content = if let Some(lvl) = &level {
            let level_prefix = match lvl.as_str() {
                "error" => "[ERROR]",
                "warn" => "[WARN]",
                "info" => "[INFO]",
                "fatal" => "[FATAL]",
            ...
            if level_prefix.is_empty() {
                ...
                content
                    .lines()
                    .filter(|line| line.contains(level_prefix))
                    .collect::<Vec<_>>()
                    .join("\n")
            }
        } else {
