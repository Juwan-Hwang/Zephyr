        ...
        if let Some(lvl) = &level {
            ...
            if lvl_lower.is_empty() || lvl_lower == "all" {
                    ...
                    > 0
                {
                    ...
                    let is_fatal = contains_bytes(&lower_buf, b"[fatal]")
                        || contains_bytes(&lower_buf, b"level=fatal");
                    let is_error = contains_bytes(&lower_buf, b"[error]")
                        || contains_bytes(&lower_buf, b"level=error");
                    let is_warn = contains_bytes(&lower_buf, b"[warn]")
                        || contains_bytes(&lower_buf, b"level=warn")
                        || contains_bytes(&lower_buf, b"[warning]")
                        || contains_bytes(&lower_buf, b"level=warning");
                    let is_info = contains_bytes(&lower_buf, b"[info]")
                        || contains_bytes(&lower_buf, b"level=info");
