...
# Explicitly reset parameters that may not be defined in other sysctl.d files
# (sysctl --system only reloads from disk, does not reset in-memory values)
...
# Reload system defaults from all remaining sysctl.d config files
# This restores buffer sizes if other config files define them
...
pub fn revert_network_optimizations(_app: AppHandle) -> Result<(), String> {
    ...
    let script = r#"do shell script "sysctl -w net.inet.tcp.msl=15000; sysctl -w net.inet.tcp.fastopen=3; sysctl -w net.inet.tcp.ecn=2" with administrator privileges"#;
