...
pub fn apply_network_optimizations(app: AppHandle) -> Result<(), String> {
    ...
    let backup_dir = backup
        ...
        .unwrap_or_default();
    let backup_file = backup.to_string_lossy().to_string();
...
# Backup current values before modifying (only if no backup exists)
mkdir -p '{backup_dir}'
if [ ! -f '{backup_file}' ]; then
cat > '{backup_file}' << BAKEOF
...
if [ -f '{backup_file}' ]; then
    sysctl -p '{backup_file}' 2>/dev/null || true
else
...
pub fn check_network_optimizations_status(app: AppHandle) -> Result<NetworkOptimStatus, String> {
    ...
    let applied = value == "1000" || has_backup;
