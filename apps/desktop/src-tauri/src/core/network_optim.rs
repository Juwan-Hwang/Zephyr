...
impl WindowsTcpOptimConfig {
    ...
    fn to_netsh_commands(&self) -> [(&'static str, Vec<&str>); 5] {
        let auto_arg = format!("autotuninglevel={}", self.autotuninglevel);
        let rto_arg = format!("initialRto={}", self.initial_rto);
        let fo_arg = format!("fastopen={}", self.fastopen);
        let ecn_arg = format!("ecncapability={}", self.ecncapability);
        [
            (
                "autotuninglevel",
                vec!["int", "tcp", "set", "global", auto_arg.leak()],
            ),
            (
                "heuristics",
                vec!["int", "tcp", "set", "heuristics", self.heuristics],
            ),
            (
                "initialRto",
                vec!["int", "tcp", "set", "global", rto_arg.leak()],
            ),
            (
                "fastopen",
                vec!["int", "tcp", "set", "global", fo_arg.leak()],
            ),
            (
                "ecncapability",
                vec!["int", "tcp", "set", "global", ecn_arg.leak()],
            ),
        ]
    }
}
