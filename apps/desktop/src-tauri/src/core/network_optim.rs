...
pub fn apply_network_optimizations(_app: AppHandle) -> Result<(), String> {
    ...
    if status.success() {
        ...
        Err("Network optimization failed (macOS)".to_owned())
    }
}
...
pub fn revert_network_optimizations(_app: AppHandle) -> Result<(), String> {
    ...
    if status.success() {
        ...
        Err("Network optimization revert failed (macOS)".to_owned())
    }
}
...
pub fn apply_network_optimizations(_app: AppHandle) -> Result<(), String> {
    ...
    for (name, args) in &optimizations {
        ...
        cmd.args(args);
        cmd.creation_flags(crate::core_manager::core::CREATE_NO_WINDOW);
        match cmd.output() {
...
pub fn revert_network_optimizations(_app: AppHandle) -> Result<(), String> {
    ...
    if failed.is_empty() {
        ...
        emit_info!(
            System,
            SYS_TUN_FAILED,
            "Windows TCP optimizations reverted (skipped: {})",
            failed.join(", ")
        );
    }
...
pub fn check_network_optimizations_status(_app: AppHandle) -> Result<NetworkOptimStatus, String> {
    ...
    let applied = text.to_lowercase().contains("fastopen") && text.to_lowercase().contains("enabled");
