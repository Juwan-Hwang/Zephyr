...
async fn run_scheduler_loop(app: AppHandle, state: Arc<SchedulerState>) {
    ...
    if !super::subscription::reconcile_global_mode_restore(&app).await {
        emit_warn!(
            ...
            "Timed out while reconciling global mode restore on startup"
        );
    }
...
async fn check_and_update_subscriptions(
    ...
    // per subscription, so lock contention must not cancel or delay the whole pass.
    if !super::subscription::reconcile_global_mode_restore_with_deadline(
        app,
    ...
    .await
    {
        emit_warn!(
            Core,
            CORE_MODE_RESTORE_FAILED,
            "Failed to reconcile global mode restore marker during scheduled update pass"
        );
    }
