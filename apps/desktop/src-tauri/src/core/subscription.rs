...
async fn try_global_mode_tier(
    ...
    if need_node_switch {
        if !check_core_current() {
            let _ = remove_global_mode_restore_marker(app);
            return Err("Global-mode: Core restarted before node switch".to_owned());
        }
    ...
    if need_mode_switch {
        if !check_core_current() {
            return Err("Global-mode: Core restarted before mode switch".to_owned());
        }
    ...
    if !check_core_current() {
        return Err("Global-mode: Core restarted after mode switch".to_owned());
    }
        ...
        match build_http_client_with_proxy(
            ...
            Ok(client) => match do_download_stream(&client, &url).await {
                ...
                Err(DownloadStreamError::Ssrf(e)) => {
                    is_ssrf_error = true;
                    Some(format!("Direct SSRF blocked: {e}"))
                }
    ...
    if result.is_none() {
            ...
            && !is_ssrf_error
        {
            ...
            if result.is_none() && !is_ssrf_error {
                ...
                let is_plain_http = sub_scheme.as_deref() == Some("http");
                if is_plain_http {
                    append_error(
                        &mut last_error,
                        "Ambient Proxy: Skipped for plain HTTP subscription URL",
                    );
                }

                let remaining = total_deadline.saturating_duration_since(std::time::Instant::now());
                if !is_plain_http
                    && (!is_sys_proxy_bypassed || !is_env_no_proxy)
