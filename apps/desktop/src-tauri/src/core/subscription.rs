...
async fn get_bounded_sys_proxy_address(timeout_dur: Duration) -> Option<String> {
    ...
    let handle = tokio::task::spawn_blocking(move || {
        ...
        let completed = std::time::Instant::now() < overall_deadline || res.is_some();
        if completed {
    ...
    if result.is_none() {
        ...
        if !user_entered_private {
            ...
            if let Some(m_url) = &mihomo_proxy_url {
                if let Some((conn_to, req_to)) = get_tier_timeout(6000, 1500) {
            ...
            if result.is_none() {
                ...
                let sys_override = crate::sys_proxy::get_sys_proxy_override();
