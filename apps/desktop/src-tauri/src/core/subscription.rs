...
async fn restore_mihomo_state(
    ...
    let target_mode = configured_mode
        .as_deref()
    ...
    if let Some(target) = target_mode.as_deref() {
        if target.eq_ignore_ascii_case("global") {
            // Mihomo remains in global mode. Do not restore a DIRECT/REJECT
            // selection while global mode is active.
            mode_is_safe = false;
            *orig_mode = None;
            *orig_global_now = None;
        } else {
...
impl std::fmt::Display for DownloadStreamError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Http(_, msg) | Self::Transport(msg) => write!(f, "{msg}"),
        }
    }
}
...
async fn download_sub_inner_raw(
    ...
    if result.is_none() {
        ...
        // User-entered private/LAN destinations, private host suffixes (.local, .internal, etc.), and single-label hosts
        // are strictly direct-only to prevent internal SSRF via proxies.
        if !user_entered_private && !is_single_label_host && !is_private_host(&host) {
            ...
            if let Some(m_url) = &mihomo_proxy_url {
                ...
                if result.is_none() && !tier2_http_error {
            ...
            if result.is_none() {
                ...
                let is_http_with_credentials = sub_scheme.as_deref() == Some("http")
                    && parsed_sub_url.as_ref().is_some_and(|u| {
                        !u.username().is_empty()
                            || u.password().is_some()
                            || u.query().is_some_and(|q| !q.trim().is_empty())
                            || (u.path() != "" && u.path() != "/")
                    });
                ...
                if is_http_with_credentials {
                    append_error(
                        &mut last_error,
                        "Ambient Proxy: Skipped because HTTP subscription URL contains credentials",
                    );
                }
