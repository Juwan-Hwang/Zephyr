        ...
        let (proxy_url, is_mihomo_proxy) = {
            ...
            let mihomo_port = {
                ...
                guard
                    .process()
                    .is_some()
                    .then(|| guard.last_proxy_port().unwrap_or(7890))
            };
            if let Some(port) = mihomo_port {
                ...
                let external = crate::sys_proxy::get_sys_proxy_address()
                    ...
                    .or_else(|| {
                        let get_env_proxy = |key: &str| {
                            std::env::var(key).ok().and_then(|v| {
                                let trimmed = v.trim();
                                (!trimmed.is_empty()).then(|| trimmed.to_owned())
                            })
                        };
                        get_env_proxy("HTTPS_PROXY")
                            .or_else(|| get_env_proxy("https_proxy"))
                            .or_else(|| get_env_proxy("ALL_PROXY"))
                            .or_else(|| get_env_proxy("all_proxy"))
                            .or_else(|| get_env_proxy("HTTP_PROXY"))
                            .or_else(|| get_env_proxy("http_proxy"))
                            .as_deref()
                            .and_then(zephyr_core::config::subscription::validate_ambient_proxy_url)
                    });
