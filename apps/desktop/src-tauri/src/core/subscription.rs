...
fn mihomo_base_api(app: &AppHandle) -> Option<(reqwest::Client, String, String)> {
    let (api_port, secret) = {
        ...
        // Guard against stopped core. In normal mode, process is Some; in macOS TUN mode,
        // process is started externally as root and set to None, but started_at and last_port
        // remain active while running.
        if guard.started_at().is_none()
            || (guard.process().is_none() && guard.last_port().is_none())
        {
            return None;
        }
...
async fn restore_mihomo_state(
    ...
    if let Some(expected_started_at) = core_started_at {
        ...
        if guard.started_at() != Some(expected_started_at) {
            drop(guard);
            remove_global_mode_restore_marker(app);
            *orig_mode = None;
...
pub fn remove_global_mode_restore_marker(app: &AppHandle) {
...
pub async fn reconcile_global_mode_restore(app: &AppHandle) {
    if mihomo_base_api(app).is_none() {
        return;
    }
    ...
    restore_mihomo_state(app, &mut orig_global_now, &mut orig_mode, None, false, None).await;
...
async fn download_sub_inner_raw(
    ...
    let has_mihomo = {
        ...
        let guard = state
            ...
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        guard.started_at().is_some() && (guard.process().is_some() || guard.last_port().is_some())
    };
        ...
        if !user_entered_private {
            ...
            let mihomo_proxy_url = {
                ...
                let guard = state
                    ...
                    .lock()
                    .unwrap_or_else(std::sync::PoisonError::into_inner);
                let running = guard.started_at().is_some()
                    && (guard.process().is_some() || guard.last_port().is_some());
                let proxy_port = guard.last_proxy_port().unwrap_or(7890);
                drop(guard);
                running.then(|| format!("http://127.0.0.1:{proxy_port}"))
            };
            ...
            if result.is_none() {
                ...
                let is_no_proxy = sub_host
                    .is_some_and(|h| zephyr_core::config::is_destination_in_no_proxy(h, sub_port));
