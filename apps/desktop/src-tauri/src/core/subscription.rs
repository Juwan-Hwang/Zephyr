...
fn is_core_running_from_guard(guard: &CoreData) -> bool {
    ...
    #[cfg(target_os = "macos")]
    {
        super::tun_manager::is_tun_mode()
            ...
            && super::core_process::has_root_mihomo()
    }
    ...
    if result.is_none() {
        ...
        if !user_entered_private && !is_single_label_host && !is_private_host(&host) {
            ...
            if let Some(m_url) = &mihomo_proxy_url {
                ...
                // Reserve at least 5500ms for Tier 3 (global mode: >=4000ms including API discovery) and Tier 4 (ambient: >=1500ms)
                let usable_tier2_ms = remaining_tier2.as_millis().saturating_sub(5500);
                if usable_tier2_ms >= 1500 {
                    let tier2_req_ms = u64::try_from(usable_tier2_ms.min(5000)).unwrap_or(u64::MAX);
