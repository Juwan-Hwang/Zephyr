        ...
        if let Some(m_url) = &mihomo_proxy_url {
            ...
            if result.is_none() {
                ...
                if remaining >= Duration::from_millis(5000) {
                    ...
                    match tokio::time::timeout(lock_wait, GLOBAL_MODE_LOCK.clone().lock_owned())
                        .await
                    {
                        Ok(lock_guard) => {
                            if let Some(orig_mode) = get_mihomo_mode(app).await {
                                    ...
                                    get_mihomo_active_node_and_global_now(app).await
                                {
                                    ...
                                    let need_node_switch =
                                        global_now.as_deref() != Some(active_node.as_str());

                                    if !need_mode_switch && !need_node_switch {
                                        append_error(
                                            &mut last_error,
                                            "Global-mode: Already in global mode with active node selected",
                                        );
                                    } else {
                                        // Re-check cumulative deadline before mutating any core state
                                        let rem_before_mutate = total_deadline
                                            .saturating_duration_since(std::time::Instant::now());
                                        if rem_before_mutate < Duration::from_millis(2500) {
                                            append_error(
                                                &mut last_error,
                                                "Global-mode: Skipped due to deadline exhaustion",
                                            );
                                        } else {
                        ...
                        Err(_timeout) => {
                            append_error(
                                &mut last_error,
                                "Global-mode: Skipped due to lock contention",
                            );
                        }
                    }
                } else {
