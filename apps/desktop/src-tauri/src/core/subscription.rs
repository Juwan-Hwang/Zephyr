...
fn is_core_running(app: &AppHandle) -> bool {
    ...
    #[cfg(target_os = "macos")]
    {
        has_port && super::tun_manager::is_tun_mode() && super::tun_manager::has_root_mihomo()
    }
...
fn mihomo_base_api(app: &AppHandle) -> Option<(reqwest::Client, String, String)> {
    ...
    if !has_process {
        ...
        {
            if port.is_none()
                || !super::tun_manager::is_tun_mode()
                || !super::tun_manager::has_root_mihomo()
            {
                return None;
            }
        }
    ...
    if result.is_none() {
        ...
        if !user_entered_private && !is_single_label_host && !is_private_host(&host) {
            ...
            if let Some(m_url) = &mihomo_proxy_url {
                ...
                // Reserve at least 3000ms for Tier 3 (global mode) and Tier 4 (ambient proxy)
                let rem_tier2_ms = u64::try_from(remaining_tier2.as_millis()).unwrap_or(u64::MAX);
                let usable_tier2_ms = rem_tier2_ms.saturating_sub(3000);
