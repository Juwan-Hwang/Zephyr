...
use zephyr_core::config::subscription::{
    ...
    is_same_loopback_proxy_endpoint, parse_content_disposition_filename, quote_short_id_values,
        ...
        match std::net::ToSocketAddrs::to_socket_addrs(&format!("{host}:{port}")) {
            ...
            Err(e) => {
                let is_same_host = attempt
                    ...
                    .is_some_and(|h| {
                        let h_norm = h.trim_end_matches('.');
                        let host_norm = host.trim_end_matches('.');
...
async fn try_global_mode_tier(
    ...
    if need_node_switch {
            ...
            .is_err()
        {
            ...
            if let Some(m) = restore_guard.orig_mode.as_deref() {
                let _ = write_global_mode_restore_marker(app, Some(m), None);
            } else {
                let _ = remove_global_mode_restore_marker(app);
            }
            return Err(DownloadStreamError::Transport(format!(
