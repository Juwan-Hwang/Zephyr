...
async fn download_sub_inner_raw(
    ...
    // 2. For public subscriptions with Mihomo running, allocate 5000ms request (2000ms connect) so
    //    slow direct endpoints have time to complete while blocked domains fast-fail on connect (2000ms)
    //    and leave ample budget (>= 7000ms) for proxy tiers.
