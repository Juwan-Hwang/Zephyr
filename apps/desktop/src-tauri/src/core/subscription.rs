...
async fn reconcile_global_mode_restore_inner(app: &AppHandle) {
    ...
    if let Some(marker_cfg) = marker.active_config {
        ...
        if let Some(curr) = current_cfg {
            if curr != marker_cfg {
                ...
                // do not restore proxy group selection or mode from a different profile.
                orig_global_now = None;
                orig_mode = None;
            }
        }
    ...
    let core_started_at = app
        .try_state::<MihomoState>()
        .and_then(|state| state.0.lock().ok().and_then(|guard| guard.started_at()));
