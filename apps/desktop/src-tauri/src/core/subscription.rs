...
pub async fn reconcile_global_mode_restore(app: &AppHandle) -> bool {
    ...
    reconcile_global_mode_restore_inner(app, None).await;
        ...
        if !user_entered_private {
            ...
            if result.is_none() {
                ...
                let remaining_override =
                    total_deadline.saturating_duration_since(std::time::Instant::now());
                let sys_override = crate::sys_proxy::get_sys_proxy_override_bounded(
                    Duration::from_millis(400).min(remaining_override / 4),
                );
