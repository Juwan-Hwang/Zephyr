...
async fn restore_mihomo_state(
    ...
    // Captured `orig_mode` reflects live state at switch time and takes precedence;
    // `configured_mode` serves as fallback when recorded mode is missing or malformed.
    let target_mode = orig_mode.clone().map(|recorded| {
        canonicalize_mode(&recorded)
            .map(String::from)
            .or_else(|| {
                configured_mode
                    .as_deref()
