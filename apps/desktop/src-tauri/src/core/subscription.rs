...
fn build_http_client_with_proxy(
    ...
    let redirect_policy = reqwest::redirect::Policy::custom(move |attempt| {
        ...
        // Resolution failures are fatal for both direct and proxy connections, ensuring
        // unresolvable or internal hostnames cannot bypass SSRF validation.
        let port = url
            .port()
...
static SYS_PROXY_DISCOVERY_LOCK: std::sync::LazyLock<tokio::sync::Mutex<()>> =
...
async fn get_bounded_sys_proxy_address(timeout_dur: Duration) -> Option<String> {
    ...
    let _lock = tokio::time::timeout(timeout_dur, SYS_PROXY_DISCOVERY_LOCK.lock())
        .await
        .ok()?;
    ...
    let res = tokio::time::timeout(
        timeout_dur,
        tokio::task::spawn_blocking(crate::sys_proxy::get_sys_proxy_address),
    )
    .await
    .ok()
    .and_then(Result::ok)
...
/// 支持在 inline 流程中受 deadline 约束，若时间耗尽则提前退出，返回尚未恢复的项，
/// 由 `ModeRestoreGuard` 的 `Drop` 实现接管在后台异步完成。
async fn restore_mihomo_state(
    app: &AppHandle,
    mut orig_global_now: Option<String>,
    mut orig_mode: Option<String>,
    deadline: Option<std::time::Instant>,
    on_drop: bool,
) -> (Option<String>, Option<String>) {
    // 1. 先恢复原模式（如 rule），使用户常规流量立即脱离 global 路由
    if let Some(orig) = orig_mode.take() {
        let mut ok = false;
        for attempt in 0..2 {
            ...
            if tokio::time::timeout(call_timeout, set_mihomo_mode(app, &orig))
                .await
                .ok()
                .flatten()
                .is_some()
    ...
    // 2. 模式恢复后再恢复原 GLOBAL 策略组选择
    if let Some(orig_node) = orig_global_now.take() {
        let mut ok = false;
        for attempt in 0..2 {
            let call_timeout = if let Some(dl) = deadline {
                let rem = dl.saturating_duration_since(std::time::Instant::now());
        ...
        if ok {
            ...
            orig_global_now = Some(orig_node);
        }
    }

    (orig_global_now, orig_mode)
}
        ...
        if orig_mode.is_some() || orig_global_now.is_some() {
            if let Ok(handle) = tokio::runtime::Handle::try_current() {
                handle.spawn(async move {
                    ...
                    restore_mihomo_state(&app, orig_global_now, orig_mode, None, true).await;
                });
            } else {
                drop(lock_guard);
                crate::emit_warn!(
...
async fn do_download_stream(
    ...
    Ok((bytes, sub_info_header, final_url, disp_filename))
}
...
async fn try_global_mode_tier(
    ...
    if remaining < Duration::from_millis(5000) {
    ...
    // 若恢复请求失败、超时或受 deadline 限制中断，guard 内部保留原值，
    // Drop 守卫将在后台继续重试恢复，并在恢复期间继续持有互斥锁。
    let (rem_node, rem_mode) = restore_mihomo_state(
        app,
        restore_guard.orig_global_now.take(),
        restore_guard.orig_mode.take(),
        Some(total_deadline),
        false,
    )
    .await;
    restore_guard.orig_global_now = rem_node;
    restore_guard.orig_mode = rem_mode;
    drop(restore_guard);
...
async fn download_sub_inner_raw(
    ...
    // If Mihomo is running, allow 3500ms for data transfer, with connect timeout capped at 1500ms so
    // blocked domains fail fast and leave sufficient budget for proxy tiers.
    // If Mihomo is stopped, allocate up to 8500ms so slow direct servers can complete without failing.
    let (direct_req_ms, direct_conn_ms) = if has_mihomo {
        (3500, 1500)
    } else {
        (8500, 3000)
    };
    ...
    if result.is_none() {
        ...
        if let Some(m_url) = &mihomo_proxy_url {
            ...
            if result.is_none() {
                match try_global_mode_tier(app, m_url, &url, user_agent.as_deref(), total_deadline)
                    .await
                {
                    Ok(data) => {
                        result = Some(data);
        ...
        if result.is_none() {
            ...
            let remaining = total_deadline.saturating_duration_since(std::time::Instant::now());
            if remaining >= Duration::from_millis(1000) {
                let sys_proxy_timeout = Duration::from_millis(800).min(remaining);
                let sys_proxy = get_bounded_sys_proxy_address(sys_proxy_timeout).await;

                let ambient_proxy_url = sys_proxy
                    .as_deref()
                    .and_then(|raw| {
                        zephyr_core::config::subscription::validate_ambient_proxy_url_for_scheme(
                            raw,
                            sub_scheme.as_deref(),
                        )
                    })
                    .or_else(|| {
                        let get_valid_env_proxy = |key: &str| {
                            std::env::var(key).ok().as_deref().and_then(|raw| {
                                zephyr_core::config::subscription::validate_ambient_proxy_url_for_scheme(
                                    raw,
                                    sub_scheme.as_deref(),
                                )
                        ...
                        if sub_scheme.as_deref() == Some("https") {
                            ...
                            get_valid_env_proxy("HTTP_PROXY")
                                .or_else(|| get_valid_env_proxy("http_proxy"))
                                .or_else(|| get_valid_env_proxy("ALL_PROXY"))
                                .or_else(|| get_valid_env_proxy("all_proxy"))
                                .or_else(|| get_valid_env_proxy("HTTPS_PROXY"))
