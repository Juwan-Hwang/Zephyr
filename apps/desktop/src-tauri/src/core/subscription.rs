...
async fn download_sub_inner_raw(
    ...
    let metadata_result = {
        ...
        // sub_info only when updating an existing subscription — avoids silently
        // resetting user-configured settings. The URL is always set to
        // final_url (the endpoint that was actually downloaded from).
        ...
        let (preserved_interval, preserved_ua, preserved_sub_info) = if overwrite {
            metadata
                .configs
                .get(&clean_name)
                .map(|m| {
