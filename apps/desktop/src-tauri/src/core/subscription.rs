...
static DNS_SEMAPHORE: std::sync::LazyLock<tokio::sync::Semaphore> =
...
async fn download_sub_inner_raw(
    ...
    let resolve_pin = if user_entered_private {
        ...
        let resolve_future = async {
            let _permit = DNS_SEMAPHORE.acquire().await.map_err(|e| e.to_string())?;
            ...
            tokio::select! {
                ...
                _ = tokio::time::sleep(Duration::from_millis(1500)) => {
                    abort_handle.abort();
                    Err(format!("DNS resolution timed out for '{host}' (1.5s)"))
                }
            }
        };
    ...
    // Outer scheduler deadline is 15s. We set a cumulative internal budget of 13s
    // across all tiers to guarantee completion, cleanup, and file saving before outer cancellation.
    let total_deadline = std::time::Instant::now() + Duration::from_millis(13000);
