...
async fn get_mihomo_active_node_and_global_now(
    ...
    let active_node = global_now
        ...
        .map(std::borrow::ToOwned::to_owned)
        .or_else(|| {
            // Look through GLOBAL's member list `all` for the first non-special candidate.
            // This ensures the chosen node or group is recognized as a valid GLOBAL member
            // by Mihomo's PUT /proxies/GLOBAL API.
            if let Some(all_members) = global_obj
                .and_then(|g| g.get("all"))
                .and_then(|a| a.as_array())
            {
                for member_val in all_members {
                    if let Some(member_name) = member_val.as_str() {
            ...
            // Fallback: Find the active node of the first selector/urltest/fallback group in proxies.
...
async fn download_sub_inner_raw(
    ...
    let resolve_pin = if user_entered_private {
        ...
        match tokio::time::timeout(Duration::from_secs(2), resolve_task).await {
            Ok(Ok(Ok(addrs))) => {
            ...
            Err(_) => {
                direct_dns_error = Some(format!("DNS resolution timed out for '{host}' (2s)"));
                None
            }
        }
    };
                                ...
                                get_mihomo_active_node_and_global_now(app).await
                            {
                                if let Some(orig_node) = global_now.as_deref() {
                                    if orig_node != active_node.as_str() {
                                        ...
                                        let _ = set_mihomo_proxy_group(app, "GLOBAL", &active_node)
                                            .await;
                                    }
                                }
                            }
