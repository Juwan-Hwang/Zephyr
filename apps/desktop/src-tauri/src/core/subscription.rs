...
use zephyr_core::config::subscription::{
    ...
    is_same_loopback_proxy_endpoint, is_single_label_host, parse_content_disposition_filename,
    quote_short_id_values, redact_url_in_string, select_global_candidate,
    ...
    let redirect_policy = reqwest::redirect::Policy::custom(move |attempt| {
        ...
        if is_private_host(&host) || is_single_label_host(&host) {
            return attempt.error(format!(
                "[SSRF_BLOCKED] Redirect to private host blocked: {host}"
            ));
        }
        ...
        match std::net::ToSocketAddrs::to_socket_addrs(&format!("{host}:{port}")) {
            Ok(addrs) => {
                for addr in addrs {
                    if is_private_ip(addr.ip()) {
                        return attempt.error(format!(
                            "[SSRF_BLOCKED] Redirect to private IP blocked: {} -> {}",
                            host,
                            addr.ip()
                        ));
                    }
            ...
            Err(e) => {
                // For requests via proxy, the proxy resolves the target remotely,
                // so local DNS resolution failure is permitted for public hostnames.
                // For direct requests, local DNS failure is fatal.
                if !via_proxy {
...
fn is_core_running(app: &AppHandle) -> bool {
...
async fn set_mihomo_proxy_group(app: &AppHandle, group: &str, name: &str) -> Result<(), bool> {
    ...
    if resp.status().is_success() {
        ...
        // Only 404 indicates group or node does not exist in Mihomo
        Err(true)
    } else {
        Err(false)
    }
...
async fn get_bounded_sys_proxy_address(timeout_dur: Duration) -> Option<String> {
    ...
    let handle = tokio::task::spawn_blocking(move || {
        ...
        let should_cache = if res.is_some() {
            true
        } else {
...
async fn restore_mihomo_state(
    ...
    // Prioritize configured_mode (user settings) so newer user mode changes are not overwritten.
    // Captured `orig_mode` serves as fallback when configured mode is missing or malformed.
    // Only calculate target_mode if orig_mode was Some (meaning this tier changed the mode).
        ...
        if target.eq_ignore_ascii_case("global") {
            ...
            *orig_mode = None;
            if let Some(orig_node) = orig_global_now.as_deref() {
                if is_special_target(orig_node) {
    ...
    if mode_is_safe && orig_mode.is_none() {
        if let Some(orig_node) = orig_global_now.as_deref() {
            let is_currently_global = configured_mode
                .as_deref()
                .map(|m| m.eq_ignore_ascii_case("global"))
                .unwrap_or(false);
            if is_currently_global && is_special_target(orig_node) {
                *orig_global_now = None;
            } else if !check_core_alive() {
                *orig_mode = None;
                *orig_global_now = None;
                return;
            } else {
...
async fn do_download_stream(
    ...
    let resp = client.get(url).send().await.map_err(|e| {
        ...
        if chain.contains("[SSRF_BLOCKED]") || chain.contains("Redirect to private") {
    ...
    let bytes = read_response_body(resp)
        .await
...
async fn try_global_mode_tier(
    ...
    let client =
        build_http_client_with_proxy(user_agent, None, Some(m_url.to_owned()), conn_to, req_to)
            .map_err(|e| {
                DownloadStreamError::Transport(format!("Global-mode client build: {e}"))
            })?;
    ...
    // 限制恢复操作在预留给后续 Tier 4 降级通道的截止时间之前完成，避免恢复重试耗尽预算。
    let ambient_reserve = Duration::from_millis(1000);
    let restore_deadline = total_deadline
        .checked_sub(ambient_reserve)
        .unwrap_or(total_deadline);
    restore_mihomo_state(
        app,
        &mut restore_guard.orig_global_now,
        &mut restore_guard.orig_mode,
        Some(restore_deadline),
        false,
        core_started_at,
    )
    .await;
...
async fn download_sub_inner_raw(
    ...
    let resolve_pin = if user_entered_private {
        ...
        if remaining < Duration::from_millis(1000) {
            ...
            let resolve_future = async {
                ...
                let permit =
                    tokio::time::timeout_at(deadline, DNS_SEMAPHORE.clone().acquire_owned())
                        .await
                        .map_err(|_err| {
                            format!(
                        "DNS resolution timeout waiting for permit for '{host}' ({dns_timeout:?})"
                    )
                        })?
                ...
                if remaining_dns.is_zero() {
                    drop(permit);
                    return Err(format!(
                        "DNS resolution timeout for '{host}' ({dns_timeout:?})"
                    ));
                }

                let (tx, rx) = tokio::sync::oneshot::channel();
                let host_port = zephyr_core::config::fetch_util::format_host_port(host_clone, port);
                let handle = tokio::task::spawn_blocking(move || {
                    let _permit = permit;
    ...
    let mut direct_http_error = false;
    let direct_error = if let Some(dns_err) = direct_dns_error {
        ...
        match build_http_client_with_proxy(
            ...
            Ok(client) => match do_download_stream(&client, &url).await {
                ...
                Err(DownloadStreamError::Http(_status, e)) => {
                    direct_http_error = true;
                    Some(format!("Direct: {e}"))
                }
    ...
    if result.is_none() {
            ...
            && !is_ssrf_error
        {
            ...
            let mihomo_proxy_url = {
                let running = is_core_running(app);
                let state = app.state::<MihomoState>();
                let guard = state
                    .0
                    .lock()
            ...
            if let Some(m_url) = &mihomo_proxy_url {
                ...
                // Reserve at least 3800ms for Tier 3 (global mode) and Tier 4 (ambient proxy)
                let rem_tier2_ms = u64::try_from(remaining_tier2.as_millis()).unwrap_or(u64::MAX);
                let usable_tier2_ms = rem_tier2_ms.saturating_sub(3800);
                let mut tier2_http_error = false;
                if usable_tier2_ms >= 1200 {
                ...
                // 注意：如果服务器已正常响应 HTTP 状态码错误（如 401/403/404 等过期/撤销错误），
                // 说明网络已达且属于订阅本身错误，绝不应切换全局模式去打扰用户全局流量。
                if result.is_none() && !tier2_http_error && !direct_http_error && !is_ssrf_error {
            ...
            if result.is_none() && !is_ssrf_error {
                ...
                let sys_override = tokio::time::timeout(
                ...
                .and_then(Result::ok)
                .flatten();
                let is_sys_proxy_bypassed = sys_override.as_deref().is_some_and(|ov| {
                    sub_host.is_some_and(|h| {
                ...
                let is_env_no_proxy = sub_host.is_some_and(|h| {
                    ...
                    && remaining >= Duration::from_millis(600)
                {
                    ...
                    for (idx, amb_url) in ambient_candidates.into_iter().enumerate() {
                        ...
                        let cands_left = u64::try_from(total_cands.saturating_sub(idx))
                            .unwrap_or(1)
                            .max(1);
                        let rem =
                            total_deadline.saturating_duration_since(std::time::Instant::now());
                        ...
                        let max_allowed = rem_ms
                            .saturating_sub(cands_left.saturating_sub(1) * 500)
                            .max(500);
                        let candidate_req_ms =
                            (rem_ms / cands_left).clamp(500, max_allowed).min(5000);
                        let candidate_conn_ms = (candidate_req_ms / 2)
                            .clamp(300, candidate_req_ms)
                            .min(1500);
