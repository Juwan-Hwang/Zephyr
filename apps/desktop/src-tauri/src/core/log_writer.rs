...
impl LogWriter {
    ...
    /// Opens the file first, then queries metadata from the file descriptor
    ...
    if let Ok(entries) = fs::read_dir(log_dir) {
        for entry in entries.flatten() {
            ...
            if let Ok(metadata) = entry.metadata() {
                if metadata.is_file() && path.extension().is_some_and(|ext| ext == "log") {
...
pub fn collect_log_files(log_dir: &Path, from_date: &str, to_date: &str) -> Vec<PathBuf> {
                ...
                if let Some(name) = path.file_name().and_then(|n| n.to_str()) {
                    if name.ends_with(".log") {
                        ...
                        if let Some(file_date) = name.get(..10) {
                            // Validate date format via byte-level checks
                            if file_date.len() == 10
                                && file_date.as_bytes().get(4) == Some(&b'-')
                                && file_date.as_bytes().get(7) == Some(&b'-')
                                && file_date.chars().all(|c| c.is_ascii_digit() || c == '-')
                                && file_date >= from_date
                                && file_date <= to_date
                            {
                                result.push(path);
    ...
    result.sort_by(|a, b| {
        ...
        let get_seq = |name: &str| -> u32 {
            // Sequenced files: "2026-06-11-2.log" -> seq=2, "2026-06-11.log" -> seq=1
            if name.len() > 14 && name.as_bytes().get(10) == Some(&b'-') {
                name.get(11..name.len() - 4)
        ...
        let a_date = a_name.get(..10).unwrap_or("");
        let b_date = b_name.get(..10).unwrap_or("");

        match a_date.cmp(b_date) {
            std::cmp::Ordering::Equal => get_seq(a_name).cmp(&get_seq(b_name)),
