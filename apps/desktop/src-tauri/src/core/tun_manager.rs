...
pub async fn restart_core_as_root(app: &AppHandle, enable_tun: bool) -> Result<String, String> {
    ...
    let script = format!(
        ...
        r#"do shell script "killall -9 zephyr-mihomo mihomo 2>/dev/null; sleep 0.3; rm -f '{escaped_log_path}'; set -C; cd '{escaped_config_dir}' && './{escaped_binary_name}' -d '.' -f 'run_config.yaml' > '{escaped_log_path}' 2>&1 &" with administrator privileges"#,
    );
