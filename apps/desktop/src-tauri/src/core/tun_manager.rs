...
pub async fn restart_core_as_root(app: &AppHandle, enable_tun: bool) -> Result<String, String> {
    ...
    //   - tcp.ecn.enable=1: enable ECN for congestion signaling without drops
    ...
    let script = format!(
        r#"do shell script "killall -9 zephyr-mihomo mihomo 2>/dev/null; sysctl -w net.inet.tcp.msl=1000 2>/dev/null; sysctl -w net.inet.tcp.fastopen=3 2>/dev/null; sysctl -w net.inet.tcp.ecn.enable=1 2>/dev/null; sleep 0.3; cd '{escaped_config_dir}' && './{escaped_binary_name}' -d '.' -f 'run_config.yaml' > '{escaped_log_path}' 2>&1 &" with administrator privileges"#,
    );
...
# Apply FQ to the default network interface (best-effort; skip if unavailable).
...
if ! sysctl -w net.ipv4.tcp_rto_min_us=5000 2>/dev/null; then
    ip route change default rto_min 5ms 2>/dev/null || true
fi
