...
pub fn get_tun_log_path() -> String {
    let root_path = std::path::Path::new("/Library/Logs/Zephyr/tun.log");
    if root_path.exists() {
        return root_path.to_string_lossy().into_owned();
    }
    if let Ok(h) = std::env::var("HOME") {
        let user_log = std::path::PathBuf::from(h).join("Library/Logs/mihomo-tun.log");
        if user_log.exists() {
            return user_log.to_string_lossy().into_owned();
        }
    }
...
pub async fn restart_core_as_root(app: &AppHandle, enable_tun: bool) -> Result<String, String> {
    ...
    // Safely truncate or create the log file as the unprivileged user before root shell writes to it,
    // avoiding an `rm -f` window in the privileged shell that could be subject to symlink races.
    ...
    let script = format!(
        ...
        r#"do shell script "killall -9 zephyr-mihomo mihomo 2>/dev/null; sleep 0.3; mkdir -p -m 0755 '/Library/Logs/Zephyr'; cd '{escaped_config_dir}' && './{escaped_binary_name}' -d '.' -f 'run_config.yaml' > '{escaped_log_path}' 2>&1 & chmod 644 '{escaped_log_path}'" with administrator privileges"#,
    );
    ...
    for _ in 0..60 {
        ...
        if script_finished && is_ready && has_root_mihomo() {
            started = true;
            break;
        }
    }
