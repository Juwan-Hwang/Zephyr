...
pub async fn restart_core_as_root(app: &AppHandle, enable_tun: bool) -> Result<String, String> {
    ...
    let script = format!(
        ...
        r#"do shell script "killall -9 zephyr-mihomo mihomo 2>/dev/null; sleep 0.3; [ -L '/Library/Logs/Zephyr' ] && rm -f '/Library/Logs/Zephyr'; mkdir -p -m 0755 '/Library/Logs/Zephyr'; chown root:wheel '/Library/Logs/Zephyr' 2>/dev/null; chmod 0755 '/Library/Logs/Zephyr'; rm -f '{escaped_log_path}'; cd '{escaped_config_dir}' && './{escaped_binary_name}' -d '.' -f 'run_config.yaml' > '{escaped_log_path}' 2>&1 & chmod 644 '{escaped_log_path}'" with administrator privileges"#,
    );
