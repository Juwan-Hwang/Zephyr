...
pub async fn restart_core_as_root(app: &AppHandle, enable_tun: bool) -> Result<String, String> {
    ...
    // CRITICAL: Escape paths for AppleScript double-quote context and shell single-quote context
    // 1. Replace \ with \\ to prevent AppleScript escape sequence interpretation (e.g. \n, \t)
    // 2. Replace " with \" to prevent breaking the outer AppleScript "do shell script \"...\"" literal
    // 3. Replace ' with '\'' for shell single-quote escaping (end quote, escaped quote, start quote)
