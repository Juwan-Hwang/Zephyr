...
pub async fn restart_core_as_root(app: &AppHandle, enable_tun: bool) -> Result<String, String> {
    ...
    // CRITICAL: Escape paths for both AppleScript double-quote context and shell single-quote context
    // 1. Replace " with \" to prevent breaking the outer AppleScript "do shell script \"...\"" literal
    // 2. Replace ' with '\'' for shell single-quote escaping (end quote, escaped quote, start quote)
    let escaped_config_dir = config_dir_str.replace('"', "\\\"").replace("'", "'\\''");
    let escaped_log_path = log_path.replace('"', "\\\"").replace("'", "'\\''");
    ...
    let escaped_binary_name = binary_name.replace('"', "\\\"").replace("'", "'\\''");
