...
pub fn get_tun_log_path() -> String {
    ...
    let create_secure_dir = |p: &std::path::Path| -> bool {
    ...
    if let Ok(h) = std::env::var("HOME") {
        let dir = std::path::PathBuf::from(h).join("Library/Logs");
        if create_secure_dir(&dir) {
            return dir.join("mihomo-tun.log").to_string_lossy().into_owned();
        }
    }

    // Fallback: process-specific private subdirectory with 0700 permissions
    let uid = unsafe { libc::getuid() };
    let temp_sub = std::env::temp_dir().join(format!("zephyr-tun-{uid}"));
    if create_secure_dir(&temp_sub) {
        return temp_sub
            .join("mihomo-tun.log")
            .to_string_lossy()
            .into_owned();
