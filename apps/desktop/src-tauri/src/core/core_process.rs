...
use super::tun_manager::{
    get_tun_log_path, has_root_mihomo, is_tun_mode, restart_core_as_root, set_tun_mode,
...
pub async fn start_core_inner(
    ...
    if !tun_active {
        ...
        let maybe_child = {
            ...
            #[cfg(target_os = "macos")]
            if child.is_some() && !has_root_mihomo() {
            ...
            if unsafe { libc::kill(pgid, libc::SIGTERM) } == 0 {
                ...
                for _ in 0..5 {
                    if let Ok(Some(_)) = child_process.try_wait() {
                        // Check if all processes in the process group have exited.
