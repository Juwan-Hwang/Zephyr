    ...
    // the core from entering a crash-restart loop due to an invalid config.
    {
        ...
        if !output.status.success() {
            ...
            if let Some(prev) = &prev_run_config {
                let _ = write_file_secure(&run_config_path, &String::from_utf8_lossy(prev));
            }
            return Err(format!(
                ...
                if e == "canceled" {
                    if let Some(prev) = prev_run_config {
                            ...
                            write_file_secure(&run_config_path, &String::from_utf8_lossy(&prev))
                        {
                            emit_warn!(
                                Config,
                                CONFIG_WRITE_FAILED,
                                "Failed to restore previous run config after root restart cancel: {write_err}"
                            );
                        }
                    }
                } else {
                    ...
                    if killed {
                        ...
                        emit_warn!(
                            Core,
                            CORE_STOP_FAILED,
                            "Failed to clean up root mihomo process after root restart failure: {err}"
                        );
                    }
                    if killed {
                        if let Ok(mut lock) =
...
pub fn stop_core_inner(app: &AppHandle, state: &MihomoState) -> Result<(), String> {
    #[cfg(target_os = "macos")]
    {
        if super::tun_manager::is_tun_mode() && super::tun_manager::has_root_mihomo() {
            super::tun_manager::smart_kill_all_mihomo_as_root().map_err(|e| {
                emit_warn!(
                    Core,
                    CORE_STOP_FAILED,
                    "Failed to kill root mihomo process on stop: {e}"
                );
                format!("Failed to kill root mihomo process on stop: {e}")
            })?;
