                    ...
                    cmd.pre_exec(move || {
                        ...
                        if libc::getppid() != parent_pid {
                            return Err(std::io::Error::new(
                                ...
                                "Parent process died before PDEATHSIG was set",
                            ));
                        }
                        Ok(())
                    });
    ...
    if let Some(mut child_process) = child {
        #[cfg(unix)]
        {
            ...
            if unsafe { libc::kill(pgid, libc::SIGTERM) } == 0 {
                // Spawn a background thread to handle graceful shutdown and
                // fallback SIGKILL to avoid blocking the async executor thread.
