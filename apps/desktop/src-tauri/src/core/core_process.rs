...
pub async fn start_core_inner(
    ...
    if is_tun_mode() {
        ...
        {
            // Use try_notify (non-blocking) so core start is not stalled
            // when the coordinator actor channel is busy.
            coordinator.try_notify(crate::network_coordinator::NetworkChangeReason::Manual);
        }
        return Ok(CoreStartResult {
            secret,
