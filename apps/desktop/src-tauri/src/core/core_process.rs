...
pub async fn start_core(
    ...
    if is_tun_mode() {
        ...
        // For TUN mode, use the config_path as-is (it's already the profile name)
        // Use case-insensitive extension check for robustness
        let active = std::path::Path::new(&config_path)
            .extension()
