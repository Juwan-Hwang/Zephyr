...
pub async fn start_core_inner(
    ...
    if is_tun_mode() {
        ...
        match lock_critical(&state.0, BackendModule::Core, codes::CORE_LOCK_FAILED) {
            Ok(mut lock) => {
            ...
            Err(e) => {
                emit_warn!(
                    Core,
                    CORE_LOCK_FAILED,
                    "TUN start: failed to acquire lock to record state: {e}"
                );
                #[cfg(target_os = "macos")]
                {
                    let _ = tokio::task::spawn_blocking(kill_all_mihomo_as_root).await;
                }
                return Err(format!("TUN start: failed to acquire core lock: {e}"));
            }
        }
