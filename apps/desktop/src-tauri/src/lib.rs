...
fn migrate_settings(mut settings: Settings, settings_file: &Path) -> Settings {
    ...
    if settings.schema_version < 1 {
        ...
        for value in settings.last_proxy_selection.values_mut() {
            // Try to parse as JSON object — if it succeeds, it's already v2
    ...
    if let Err(e) = core_manager::write_file_secure(settings_file, &json_str) {
        ...
        // Back up the original file so the user can recover manually
        let ts = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap_or_default()
            .as_millis();
        let backup_path = settings_file.with_extension(format!("pre-migration.{ts}.json"));
        let _ = std::fs::rename(settings_file, &backup_path);
        eprintln!("[Settings] Original settings backed up to {backup_path:?}");
    }
