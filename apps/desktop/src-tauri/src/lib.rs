...
pub(crate) struct WebviewHealth {
    ...
    /// `now_unix_ms()` (app startup time), **never** `0`, so the cold-start
...
impl Default for WebviewHealth {
    fn default() -> Self {
        Self {
            ...
            last_heartbeat_ms: AtomicI64::new(now_unix_ms()),
...
pub(crate) fn is_webview_alive(app: &tauri::AppHandle) -> bool {
    if let Some(health) = app.try_state::<WebviewHealth>() {
        ...
        let elapsed_ms = now_unix_ms() - last;
        let timeout_ms = i64::try_from(HEARTBEAT_TIMEOUT_SECS).unwrap_or(i64::MAX) * 1000;
...
pub(crate) fn show_or_recreate_main_window(app: &tauri::AppHandle) {
    ...
    if let Some(window) = app.get_webview_window("main") {
        ...
        // Webview is dead (heartbeat stale) — destroy zombie first.
        eprintln!("[lib] Webview heartbeat stale — recreating window");
        let _ = window.destroy();
    }

    // Slow path: reconstruct the window (either it didn't exist, or we just
    // destroyed the zombie). Guard against concurrent reconstruction.
