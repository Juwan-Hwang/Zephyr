...
pub(crate) fn show_or_recreate_main_window(app: &tauri::AppHandle) {
    ...
    if let Some(window) = app.get_webview_window("main") {
        ...
        if is_webview_alive(app) {
            ...
            // If the window was hidden, the webview might have crashed while
            // suspended (heartbeat paused). Spawn a deferred check: if the
            // heartbeat isn't updated within 2 s, the webview is dead —
            // recreate.
            if was_hidden {
                let heartbeat_before = if let Some(health) = app.try_state::<WebviewHealth>() {
                    health.last_heartbeat_ms.load(Ordering::Relaxed)
                } else {
                    0
                };
                ...
                tauri::async_runtime::spawn(async move {
                    ...
                    // Abort if the user has hidden the window again during
                    // the wait — we must not force it back up.
                    ...
                    let heartbeat_after =
                        if let Some(health) = app_clone.try_state::<WebviewHealth>() {
                            health.last_heartbeat_ms.load(Ordering::Relaxed)
                        } else {
                            0
                        };
                    if heartbeat_after == heartbeat_before {
                        eprintln!(
                            "[lib] Webview failed to resume heartbeat after show — recreating"
                        );
                        let app_clone2 = app_clone.clone();
                        let _ = app_clone.run_on_main_thread(move || {
                            show_or_recreate_main_window(&app_clone2);
                        });
                    }
                });
            }
