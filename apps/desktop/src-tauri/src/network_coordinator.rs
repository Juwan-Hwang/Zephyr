...
pub async fn get_network_state(app: AppHandle) -> Result<NetworkState, String> {
    ...
    let mut state = match cached {
        Some(s) => s,
        None => tokio::task::spawn_blocking(detect_network_state)
            .await
            .map_err(|e| format!("Network detection task failed: {e}"))?,
    };
...
pub async fn notify_network_change(app: AppHandle, source: Option<String>) -> Result<(), String> {
    ...
    let is_manual = reason == NetworkChangeReason::Manual;
    ...
    .await
    {
        Ok(Ok(())) => Ok(()),
        Ok(Err(_)) => {
            // If a Manual notification was dropped (channel closed), recover
            // by invalidating applied_state and resetting retry counters,
            // matching the timeout recovery in notify_core_started.
            if is_manual {
