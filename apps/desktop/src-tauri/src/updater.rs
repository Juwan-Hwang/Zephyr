...
fn parse_tag_from_atom_feed(xml: &str) -> Result<String, String> {
    ...
    loop {
        ...
        let tag = extract_title_from_entry(entry)?;
...
fn construct_release_from_tag(tag: &str) -> Result<GithubRelease, String> {
    ...
    let ext = if os_tag == "windows" || os_tag == "darwin" {
        "zip"
    } else {
        "gz"
    };
    ...
    // v3/compatible variants are only published for Linux and Windows.
    // macOS (darwin) only has plain assets — generating variants would
    // cause 404 errors when the Atom feed path is used.
    if arch_tag == "amd64" && os_tag != "darwin" {
...
mod tests {
    ...
    fn test_construct_release_amd64_v3() {
        ...
        // v3/compatible variants are only generated for non-darwin amd64.
        if arch_tag == "amd64" && os_tag != "darwin" {
        ...
        } else if arch_tag == "amd64" && os_tag == "darwin" {
            // macOS: only plain assets.
            assert_eq!(release.assets.len(), 1);
            let first = release.assets.first().unwrap();
            assert!(!first.name.contains("-v3-"));
            assert!(!first.name.contains("-compatible-"));
        }
    }
