...
/// Parse the first `<entry>`'s `<title>` from an Atom feed XML string.
///
/// Handles `<entry>` and `<entry xmlns="...">` variants, as well as
/// `<title>` and `<title type="...">` variants.
/// Returns the trimmed tag text, or `Err` if the feed is malformed.
fn parse_tag_from_atom_feed(xml: &str) -> Result<String, String> {
    // Lightweight XML extraction: find the first <entry → <title>text</title>
    // We search for "<entry" (not "<entry>") to tolerate attributes/namespaces.
    let entry_start = xml
        .find("<entry")
        .ok_or_else(|| "No <entry> in Atom feed".to_owned())?;
