...
fn handle_process_failed(
    app: &AppHandle,
    event_args: Option<&webview2_com::Microsoft::Web::WebView2::Win32::ICoreWebView2ProcessFailedEventArgs>,
) {
...
fn recreate_window(app: &AppHandle) {
    if !crate::try_acquire_reconstruct_gate(app) {
    ...
    if let Err(e) = crate::recreate_main_window(app) {
    ...
    crate::release_reconstruct_gate(app);
}
