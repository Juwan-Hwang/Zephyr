...
fn recreate_window(app: &AppHandle) {
    ...
    let _ = app.run_on_main_thread(move || {
        ...
        // `None` (no window)  => hidden (false) — conservative: don't pop
        //         up the recovered window if we don't know its prior state.
        // `Err`  (query failed) => visible (true)  — safe default so the
        //         recreated window is surfaced rather than disappearing.
        // `Ok(false)`          => hidden (false) — preserve hidden-to-tray.
