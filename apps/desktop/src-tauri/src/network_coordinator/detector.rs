...
///
/// Cache TTL — kept below the 5-second polling interval so polling
/// ticks always detect a fresh state rather than a potentially stale one.
const STATE_CACHE_TTL_SECS: u64 = 4;
    ...
    while attempts < 3 {
        ...
        match result {
            ...
            111 => {
                ...
                unsafe { std::alloc::dealloc(ptr, layout) };
                size = ret.max(size.saturating_mul(2));
            }
            _ => {
                // SAFETY: deallocate the buffer before returning.
                unsafe { std::alloc::dealloc(ptr, layout) };
    ...
    if let Ok(entries) = std::fs::read_dir("/sys/class/net") {
        for entry in entries.flatten() {
            ...
            if (name.starts_with("eth") || name.starts_with("en"))
                && !entry.path().join("wireless").exists()
            {
