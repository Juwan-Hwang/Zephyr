...
/// single 5-second window can serve multiple callers without re-spawning
/// `netsh`, `networksetup`, `ifconfig`, or `GetAdaptersAddresses`.
const STATE_CACHE_TTL_SECS: u64 = 5;
...
/// Test-only accessor: returns `true` if the state cache is empty.
#[cfg(test)]
#[must_use]
pub fn is_cache_empty() -> bool {
    STATE_CACHE.lock().map(|c| c.is_none()).unwrap_or(true)
}
