    ...
    if let Some(out) = output {
        if out.status.success() {
            ...
            for line in text.lines() {
                ...
                // "disconnected" contains the substring "connected", so
                // check for the exact word boundary to avoid false positive.
                if trimmed.ends_with("connected")
                    && !trimmed.ends_with("disconnected")
