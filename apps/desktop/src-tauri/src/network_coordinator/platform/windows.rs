...
pub fn start_windows_listener(event_tx: Sender<NetworkChangeReason>) {
            ...
            loop {
                ...
                if ret == 0 {
                    ...
                    if event_tx.blocking_send(reason).is_err() {
                        ...
                        // Reset the guard so a future coordinator can restart.
                        WATCHER_STARTED.store(false, Ordering::SeqCst);
                        break;
                    }
                } else {
                    if event_tx.is_closed() {
                        // Coordinator dropped the receiver; stop retrying.
                        WATCHER_STARTED.store(false, Ordering::SeqCst);
                        break;
                    }
                    emit_warn!(
        ...
        })
    {
        ...
        // Release the guard so a later call can retry the spawn.
        WATCHER_STARTED.store(false, Ordering::SeqCst);
    }
}
