...
#[test]
fn test_mask_ssid_sanitizes_control_chars() {
    // SSID with bidi override, zero-width, and BOM — sanitize_text must strip these
    // before masking, preventing log injection and text reordering.
    let state = NetworkState {
        interface_type: InterfaceType::Wifi,
        is_connected: true,
        ssid: Some("\u{202e}Home\u{200b}WiFi\u{feff}".to_owned()),
    };
    // After sanitization, the SSID should be "HomeWiFi" (8 chars).
    // mask_ssid shows first 2 + *** + last 2 = "Ho***Fi"
    assert_eq!(state.masked(), "wifi(Ho***Fi)");
}
...
async fn test_debounce_burst_events_single_evaluation() {
    ...
    let handle = tokio::spawn(async move {
        ...
        // Run the loop for 500ms — long enough for a second debounce
        // to fire if the implementation is broken. With correct
        // debounce, only one eval should occur after the burst.
...
fn test_reconciliation_retry_on_apply_failure_semantic() {
    ...
    // 3. Apply #1 FAILS — applied_state is NOT updated; it remains Home-WiFi
    assert_eq!(
        applied_state
            .lock()
            .unwrap()
    ...
    // 5. Apply #2 SUCCEEDS — update applied_state
