...
fn test_reconciliation_retry_on_apply_failure_semantic() {
    ...
    *observed_state.lock().unwrap() = fresh_state.clone();
    ...
    // 2. Reconciliation check detects unaligned state
    let needs_reconcile = applied_state.lock().unwrap().as_ref() != Some(&fresh_state);
    assert!(
        needs_reconcile,
    ...
    let retry_needed = applied_state.lock().unwrap().as_ref() != Some(&sampled_on_next_tick);
    assert!(
        retry_needed,
    ...
    let no_op = applied_state.lock().unwrap().as_ref() == Some(&sampled_on_next_tick);
...
fn test_ping_pong_state_reversion_no_op() {
    ...
    if let Ok(guard) = current_state.lock() {
        if *guard != sampled_ground_truth {
            state_changed = true;
        }
    }

    assert!(!state_changed, "Ping-pong reversion back to original state should be detected as No-Op with 0 apply triggers");
}
