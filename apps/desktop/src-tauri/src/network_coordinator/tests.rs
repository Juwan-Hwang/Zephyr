...
// Use multi_thread flavor to guarantee the Barrier(2) can have two
// spawn_blocking closures running simultaneously on different threads.
// On current_thread runtime the closures could run sequentially on the
// same pooled worker, making the test pass even without thread isolation.
