...
fn trigger_single_flight_apply(
    ...
    // `Err`. Clear `is_applying` and `pending_rerun` so the coordinator is
    // not permanently stuck and the next trigger can start fresh.
    let supervisor_is_applying = Arc::clone(&shared.is_applying);
    let supervisor_pending_rerun = Arc::clone(&shared.pending_rerun);
    tauri::async_runtime::spawn(async move {
        if worker_handle.await.is_err() {
            supervisor_is_applying.store(false, Ordering::SeqCst);
            supervisor_pending_rerun.store(false, Ordering::SeqCst);
        }
    });
}
