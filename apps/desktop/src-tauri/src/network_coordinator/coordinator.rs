...
fn trigger_single_flight_apply(
    ...
    tauri::async_runtime::spawn(async move {
        ...
        loop {
            ...
            // consume it.
            //
            // Note: if `run_prism_apply_task` panics, Tokio's default
            // `unhandled_panic` behavior catches the panic (does NOT abort
            // the runtime), so this code will NOT execute and `is_applying`
            // would remain stuck. In practice, `run_prism_apply_task` has no
            // panic-prone code paths (all errors are handled as `ApplyOutcome`
            // variants). If panics become a concern, wrap the call in
            // `AssertUnwindSafe(...).catch_unwind()` or add a timeout-based
            // watchdog in the polling tick.
