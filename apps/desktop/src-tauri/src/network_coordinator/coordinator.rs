        ...
        tokio::select! {
            ...
            recv_result = event_rx.recv() => {
                if let Some(reason) = recv_result {
                    ...
                    // failure counter so polling-driven retries resume.
            ...
            _ = polling_interval.tick() => {
                ...
                // Trigger debounce if observed state changed OR if desired state differs from
                // applied state (reconciliation retry). Suppress polling-driven retries after
                // too many consecutive failures to avoid unbounded recompile loops; an
                // explicit event (native, resume, online, manual) will reset the counter.
                ...
                if unaligned && debounce_deadline.is_none() && failures < MAX_CONSECUTIVE_FAILURES {
...
fn trigger_single_flight_apply(
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            if apply_result {
                // Apply succeeded: mark applied_state and reset failure counter
