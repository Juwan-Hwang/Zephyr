...
impl NetworkCoordinatorHandle {
    ...
    /// # Panics
    ///
    /// Panics if called from a thread that is driving a Tokio runtime
    /// (e.g. inside an `async` task or a `#[tokio::main]` context).
    /// Use `notify` in async contexts instead.
    ...
    loop {
        tokio::select! {
            ...
            recv_result = event_rx.recv() => {
                if let Some(reason) = recv_result {
                    ...
                    let debounce_ms = match &reason {
                        NetworkChangeReason::Resume => {
                            // Force reconciliation after sleep — even if the
                            // network snapshot is unchanged, the core may have
                            // restarted and Prism rules need re-application.
                            if let Ok(mut guard) = shared.applied_state.lock() {
                                *guard = None;
                            }
                            2500 // Longer window for Wi-Fi reconnect on system wake
                        }
            ...
            _ = polling_interval.tick() => {
                ...
                let needs_retry = needs_reconciliation(&last_applied, &sampled);
                ...
                if should_trigger {
                    ...
                    last_reason = Some(reason);
                    let now = Instant::now();
                    debounce_anchor.get_or_insert(now);
...
async fn detect_via_spawn_blocking(detector: fn() -> NetworkState) -> Option<NetworkState> {
    // Wrap in a timeout so a hung subprocess (netsh, networksetup, nmcli)
    // cannot stall the coordinator actor loop indefinitely.
    match tokio::time::timeout(
        Duration::from_secs(5),
        tokio::task::spawn_blocking(detector),
    )
    .await
    {
        ...
        Err(_) => {
            emit_warn!(
                System,
                SYS_NETWORK_COORDINATOR_ERROR,
                "Network detection timed out after 5s. Skipping this tick."
            );
            None
        }
    }
...
fn trigger_single_flight_apply(
        ...
        .is_err()
    {
        ...
        if shared
            ...
            .is_ok()
        {
            // The second CAS succeeded — the apply will run now, so
            // correct the prematurely-incremented pending_reruns counter.
            if let Ok(mut m) = shared.metrics.lock() {
                m.pending_reruns = m.pending_reruns.saturating_sub(1);
            }
        ...
        if let Some(mihomo_state) = app.try_state::<crate::MihomoState>() {
            if let Ok(mut guard) = mihomo_state.0.lock() {
                ...
                emit_warn!(
                    System,
                    SYS_NETWORK_COORDINATOR_ERROR,
                    "MihomoState mutex poisoned — cannot verify core running state"
                );
                false
            }
        } else {
