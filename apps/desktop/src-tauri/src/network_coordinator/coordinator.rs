...
fn trigger_single_flight_apply(
        ...
        .is_err()
    {
        ...
        // The worker releases is_applying (store false) and then checks
        // pending_rerun. If we set pending_rerun before the worker's check,
        // the worker will consume it. If we set it after the worker's check,
        // the re-check below catches the released is_applying and starts a
        // new worker. This closes the lost-wakeup window.
        ...
        if is_applying
            ...
            .is_ok()
        {
            pending_rerun.store(false, Ordering::SeqCst);
        } else {
            return;
        }
    }
