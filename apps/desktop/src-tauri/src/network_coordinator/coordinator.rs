    ...
    loop {
        tokio::select! {
            ...
            _ = polling_interval.tick() => {
                ...
                //      It also bypasses the deferred backoff: a real transition while
                //      the core is stopped must not be delayed by up to 60 seconds.
                ...
                // A genuine state transition resets the failure counter so that
                // retries for the new state get the full budget.
                if state_changed {
                    shared.consecutive_failures.store(0, Ordering::SeqCst);
                }
                ...
                // 2^deferred seconds, capped at 60s. Real state changes are exempt.
