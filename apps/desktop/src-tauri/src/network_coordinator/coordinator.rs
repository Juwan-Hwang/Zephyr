...
pub fn start_coordinator(app: &AppHandle) -> NetworkCoordinatorHandle {
    ...
    let observed_state = Arc::new(Mutex::new(initial_state.clone()));
    // Seed applied_state with the initial network state so the first polling
    // cycle is a no-op. The frontend runs prism.apply() during startup which
    // compiles rules for the current network; if that fails, the next network
    // change still triggers reconciliation via the normal path.
    let applied_state = Arc::new(Mutex::new(Some(initial_state)));
    ...
    let observed_state_actor = Arc::clone(&observed_state);
    let applied_state_actor = Arc::clone(&applied_state);
    let is_applying_actor = Arc::clone(&is_applying);
    let pending_rerun_actor = Arc::clone(&pending_rerun);
    let metrics_actor = Arc::clone(&metrics);

    tauri::async_runtime::spawn(async move {
        coordinator_actor_loop(
            app_clone,
            event_rx,
            observed_state_actor,
            applied_state_actor,
            is_applying_actor,
            pending_rerun_actor,
            metrics_actor,
...
async fn coordinator_actor_loop(
    ...
    loop {
        tokio::select! {
            // 1. Inbound event received from OS listener, resume, online, or manual trigger
            Some(reason) = event_rx.recv() => {
            ...
            _ = polling_interval.tick() => {
                ...
                let last_observed = observed_state
                    .lock()
                    .map(|g| g.clone())
                    .unwrap_or_default();
                let last_applied = applied_state
                    .lock()
                    .ok()
                    .and_then(|g| g.clone());

                // Trigger debounce if observed state changed OR if desired state differs from applied state (reconciliation retry)
                let unaligned = sampled != last_observed || last_applied.as_ref() != Some(&sampled);
                if unaligned && debounce_deadline.is_none() {
...
fn handle_stable_network_check(
    ...
    let (old_observed, observed_changed) = {
        let mut guard = match observed_state.lock() {
            Ok(g) => g,
            Err(_) => return,
        };
    ...
    let needs_reconcile = {
        let applied_guard = match applied_state.lock() {
            Ok(g) => g,
            Err(_) => return,
        };
        applied_guard.as_ref() != Some(&fresh_state)
    };
...
fn trigger_single_flight_apply(
    ...
    let app_clone = app.clone();
    let applied_state_clone = Arc::clone(applied_state);
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            let apply_result =
                run_prism_apply_task(&app_clone, &current_reason, &active_target).await;

            if apply_result {
                // Apply succeeded: mark applied_state
            ...
            } else if let Ok(mut m) = metrics_clone.lock() {
                m.apply_failed += 1;
            }
...
pub fn validate_http_status(http_status: Option<u16>) -> bool {
    http_status.is_some_and(|code| (200..=299).contains(&code))
}
...
/// Executes Prism rule compilation and Mihomo hot-reload. Returns `true` if successful or skipped cleanly.
