    ...
    loop {
        tokio::select! {
            ...
            _ = polling_interval.tick() => {
                ...
                let sampled = tokio::task::spawn_blocking(detect_network_state)
                    .await
                    .unwrap_or_default();
...
async fn handle_stable_network_check(
    ...
    let fresh_state = tokio::task::spawn_blocking(super::detector::detect_network_state_uncached)
        .await
        .unwrap_or_default();
    ...
    let worker_handle = tauri::async_runtime::spawn(async move {
        ...
        loop {
            ...
            if pending_rerun_clone.swap(false, Ordering::SeqCst) {
                ...
                let latest =
                    tokio::task::spawn_blocking(super::detector::detect_network_state_uncached)
                        .await
                        .unwrap_or_default();
