...
fn run_cmd_bounded(mut cmd: Command, timeout: std::time::Duration) -> Option<std::process::Output> {
    ...
    loop {
        match child.try_wait() {
            Ok(Some(status)) => {
                // The child exited and was reaped, so the pipe write end is closed and
                // `read_to_end` in the reader thread is guaranteed to return.
                let _ = reader_handle.join();
                let stdout_bytes = rx.try_recv().unwrap_or_default();
...
fn push_normalized_proxy_part(parts: &mut Vec<String>, raw: &str, tag: &str) {
    ...
    let part = if normalized.contains('=') {
        normalized
    } else {
