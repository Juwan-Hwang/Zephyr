...
fn run_cmd_bounded(mut cmd: Command, timeout: std::time::Duration) -> Option<std::process::Output> {
    ...
    loop {
        match child.try_wait() {
            Ok(Some(status)) => {
                // The child has exited and was reaped, so the reader thread only drains
                // buffered pipe data. Avoid signaling the process group post-reap to
                // prevent killing recycled PIDs.
                let stdout_bytes = rx
