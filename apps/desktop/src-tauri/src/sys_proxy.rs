...
pub fn get_sys_proxy_override_bounded(timeout: std::time::Duration) -> Option<String> {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let order = match desktop {
            LinuxDesktop::Kde => [1, 0, 2],
            LinuxDesktop::Xfce => [2, 0, 1],
            LinuxDesktop::Gnome | LinuxDesktop::Other => [0, 1, 2],
        };

        for idx in order {
            let res = match idx {
                0 => query_gnome_override(),
                1 => query_kde_override(),
                _ => query_xfce_override(),
            };
            if let Some(r) = res {
                return Some(r);
            }
