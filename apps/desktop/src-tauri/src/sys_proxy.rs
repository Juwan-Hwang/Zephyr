...
pub fn get_sys_proxy_address_with_deadline(deadline: std::time::Instant) -> Option<String> {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let query_gnome = |parts: &mut Vec<String>| {
        ...
        let query_kde = |parts: &mut Vec<String>| {
                    ...
                    get_kde_cmd_bounded(std::time::Duration::from_millis(800).min(rem_kde))
                {
                    ...
                    if rem_cmd >= std::time::Duration::from_millis(50) {
                        ...
                        if let Some(output) = run_cmd_bounded(
                            k_cmd,
                            std::time::Duration::from_millis(800).min(rem_cmd),
                        ) {
                                ...
                                let mut check_kde_proxy = |key: &str, tag: &str| {
                                    ...
                                    if let Some(proxy_output) = run_cmd_bounded(
                                        ...
                                        if proxy_output.status.success() {
                                            let raw_proxy =
                                                String::from_utf8_lossy(&proxy_output.stdout);
        ...
        let query_xfce = |parts: &mut Vec<String>| {
        ...
        match desktop {
            LinuxDesktop::Kde => {
                query_kde(&mut all_parts);
                if all_parts.is_empty() {
                    query_gnome(&mut all_parts);
                    query_xfce(&mut all_parts);
                }
            }
            LinuxDesktop::Xfce => {
                query_xfce(&mut all_parts);
