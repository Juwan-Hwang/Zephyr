...
fn get_network_services() -> Vec<String> {
    get_network_services_bounded(std::time::Duration::from_secs(5))
}
    ...
    loop {
        match child.try_wait() {
            Ok(Some(status)) => {
                ...
                let wait_time = std::time::Duration::from_millis(150).min(rem);
                if !wait_time.is_zero() {
                    if let Ok(bytes) = rx.recv_timeout(wait_time) {
                ...
                let rem_after_kill = timeout.saturating_sub(start.elapsed());
                let drain_time = std::time::Duration::from_millis(20).min(rem_after_kill);
                if !drain_time.is_zero() {
                    if let Ok(bytes) = rx.recv_timeout(drain_time) {
                        let _ = reader_handle.join();
                        return Some(std::process::Output {
                            status,
                            stdout: bytes,
...
fn get_kde_cmd() -> Option<(&'static str, &'static str)> {
    get_kde_cmd_bounded(std::time::Duration::from_secs(5))
}

#[cfg(target_os = "linux")]
/// Checks if GNOME proxy configuration is available by verifying that `gsettings`
/// successfully queries the `org.gnome.system.proxy` schema. Systems without GNOME
/// installed (such as KDE/XFCE distros that still bundle the `gsettings` binary)
/// exit non-zero and are correctly skipped.
...
#[cfg(target_os = "linux")]
fn has_gnome() -> bool {
    has_gnome_bounded(std::time::Duration::from_secs(5))
}
...
#[cfg(target_os = "linux")]
fn has_xfce() -> bool {
    has_xfce_bounded(std::time::Duration::from_secs(5))
}
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        for service in services {
            ...
            if let Some((h, p)) = parse_proxy("-getwebproxy") {
                service_parts.push(format!("http={h}:{p}"));
            }
            if let Some((h, p)) = parse_proxy("-getsecurewebproxy") {
                service_parts.push(format!("https={h}:{p}"));
            }
            if let Some((h, p)) = parse_proxy("-getsocksfirewallproxy") {
                service_parts.push(format!("socks={h}:{p}"));
            }

            if !service_parts.is_empty() {
                return Some(service_parts.join(";"));
            }
        }
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let query_gnome = |parts: &mut Vec<String>| -> DesktopQueryResult {
            ...
            if mode == "'none'" || mode != "'manual'" {
                return DesktopQueryResult::ExplicitlyDisabled;
            }
            ...
            let mut check_gnome_proxy = |schema: &str, tag: &str| {
                    ...
                    run_cmd_bounded(h_cmd, std::time::Duration::from_millis(800).min(rem))
                {
                    ...
                    if !raw_host.is_empty() {
                        ...
                        let rem_p = deadline.saturating_duration_since(std::time::Instant::now());
                        ...
                        if let Some(port_output) =
                            run_cmd_bounded(p_cmd, std::time::Duration::from_millis(800).min(rem_p))
                        {
                    ...
                    run_cmd_bounded(cmd, std::time::Duration::from_millis(800).min(rem))
                {
                    if proxy_output.status.success() {
                        ...
                        if !trimmed.is_empty() {
                            ...
                            push_normalized_proxy_part(parts, &normalized, tag);
                        }
                    }
                }
            };
        ...
        let query_xfce = |parts: &mut Vec<String>| -> DesktopQueryResult {
            ...
            let mut check_xfce_proxy = |prop: &str, tag: &str| {
                    ...
                    run_cmd_bounded(cmd, std::time::Duration::from_millis(800).min(rem))
                {
                    if output.status.success() {
                        ...
                        if !trimmed.is_empty() {
                            push_normalized_proxy_part(parts, trimmed, tag);
                        }
                    }
                }
            };
        ...
        match desktop {
            LinuxDesktop::Kde => {
                let res = query_kde(&mut all_parts);
                if res == DesktopQueryResult::Failed && all_parts.is_empty() {
                    let _ = query_gnome(&mut all_parts);
                    let _ = query_xfce(&mut all_parts);
                }
            }
            LinuxDesktop::Xfce => {
                let res = query_xfce(&mut all_parts);
                if res == DesktopQueryResult::Failed && all_parts.is_empty() {
                    let _ = query_gnome(&mut all_parts);
                    let _ = query_kde(&mut all_parts);
                }
            }
            LinuxDesktop::Gnome => {
                let res = query_gnome(&mut all_parts);
                if res == DesktopQueryResult::Failed && all_parts.is_empty() {
                    let _ = query_kde(&mut all_parts);
                    let _ = query_xfce(&mut all_parts);
                }
...
pub fn get_sys_proxy_override_bounded(timeout: std::time::Duration) -> Option<String> {
    #[cfg(target_os = "windows")]
    {
        if timeout < std::time::Duration::from_millis(50) {
            return None;
        }
        let hkcu = RegKey::predef(HKEY_CURRENT_USER);
        let path = "Software\\Microsoft\\Windows\\CurrentVersion\\Internet Settings";
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        for service in services {
            ...
            // Only collect bypass domains if this service actually has a proxy enabled
            let has_enabled_proxy = {
                let mut enabled = false;
                for proto in [
                    "-getwebproxy",
                    "-getsecurewebproxy",
                    ...
                    let rem_p = timeout.saturating_sub(start.elapsed());
                    if rem_p < std::time::Duration::from_millis(50) {
                        break;
                    }
                    let mut cmd = Command::new("networksetup");
                    cmd.args([proto, &service]);
                    if let Some(output) =
                        run_cmd_bounded(cmd, std::time::Duration::from_millis(400).min(rem_p))
                    {
                        if output.status.success() {
                            let text = String::from_utf8_lossy(&output.stdout);
                            if text.lines().any(|l| {
                                ...
                                if let Some((k, v)) = t.split_once(':') {
                                    k.trim().eq_ignore_ascii_case("Enabled") && v.contains("Yes")
                                } else {
                                    false
                                }
                            }) {
            ...
            if !has_enabled_proxy {
                continue;
            }

            let rem_bypass = timeout.saturating_sub(start.elapsed());
            if rem_bypass < std::time::Duration::from_millis(50) {
                break;
            }
            let mut cmd = Command::new("networksetup");
            cmd.args(["-getproxybypassdomains", &service]);
            if let Some(output) =
                run_cmd_bounded(cmd, std::time::Duration::from_millis(400).min(rem_bypass))
            {
                if output.status.success() {
                    let text = String::from_utf8_lossy(&output.stdout);
                    for line in text.lines() {
                        let trimmed = line.trim();
                        if !trimmed.is_empty()
                            && !trimmed.starts_with("There aren")
                            && !trimmed.contains(char::is_whitespace)
                            && !all_domains.iter().any(|d| d.eq_ignore_ascii_case(trimmed))
                        {
                            all_domains.push(trimmed.to_owned());
                        }
                    }
                }
            }
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let query_gnome_override = || -> Option<String> {
            let rem = timeout.saturating_sub(start.elapsed());
            if rem >= std::time::Duration::from_millis(50) {
                let mut m_cmd = Command::new("gsettings");
                m_cmd.args(["get", "org.gnome.system.proxy", "mode"]);
                        ...
                        && String::from_utf8_lossy(&m_out.stdout).trim() == "'manual'"
                    {
                        ...
                        if rem_g >= std::time::Duration::from_millis(50) {
                            ...
                            if let Some(output) = run_cmd_bounded(
                                g_cmd,
                                std::time::Duration::from_millis(400).min(rem_g),
                            ) {
                                if output.status.success() {
                                    let text = String::from_utf8_lossy(&output.stdout);
        ...
        let query_kde_override = || -> Option<String> {
            let rem = timeout.saturating_sub(start.elapsed());
            if rem >= std::time::Duration::from_millis(50) {
                    ...
                    get_kde_cmd_bounded(std::time::Duration::from_millis(400).min(rem))
                {
                    ...
                    if rem_k >= std::time::Duration::from_millis(50) {
                        ...
                        if let Some(type_out) = run_cmd_bounded(
                            type_cmd,
                            std::time::Duration::from_millis(400).min(rem_k),
                        ) {
        ...
        let query_xfce_override = || -> Option<String> {
            let rem = timeout.saturating_sub(start.elapsed());
            if rem >= std::time::Duration::from_millis(50)
                && has_xfce_bounded(std::time::Duration::from_millis(400).min(rem))
            {
                ...
                if rem_np >= std::time::Duration::from_millis(50) {
                    ...
                    if let Some(output) =
                        run_cmd_bounded(x_cmd, std::time::Duration::from_millis(400).min(rem_np))
                    {
                        if output.status.success() {
                            let text = String::from_utf8_lossy(&output.stdout)
                                .trim()
        ...
        match desktop {
            LinuxDesktop::Gnome => query_gnome_override(),
            LinuxDesktop::Kde => query_kde_override(),
            LinuxDesktop::Xfce => query_xfce_override(),
            LinuxDesktop::Other => query_gnome_override()
                .or_else(query_kde_override)
                .or_else(query_xfce_override),
        }
    }
}
