    ...
    loop {
        match child.try_wait() {
            Ok(Some(status)) => {
                ...
                let wait_time = remaining.min(std::time::Duration::from_millis(150));
...
pub fn get_sys_proxy_override_bounded(timeout: std::time::Duration) -> Option<String> {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        if rem >= std::time::Duration::from_millis(50) {
            ...
            if let Some(output) =
                run_cmd_bounded(g_cmd, std::time::Duration::from_millis(400).min(rem))
            {
                if output.status.success() {
                    let text = String::from_utf8_lossy(&output.stdout);
                    let trimmed = text.trim();
                    if trimmed.starts_with('[') && trimmed.ends_with(']') {
                        let items: Vec<&str> = trimmed[1..trimmed.len().saturating_sub(1)]
                            .split(',')
        ...
        if rem >= std::time::Duration::from_millis(50) {
                ...
                get_kde_cmd_bounded(std::time::Duration::from_millis(400).min(rem))
            {
                ...
                if rem_k >= std::time::Duration::from_millis(50) {
                    ...
                    if let Some(output) =
                        run_cmd_bounded(k_cmd, std::time::Duration::from_millis(400).min(rem_k))
                    {
                        if output.status.success() {
            ...
            && has_xfce_bounded(std::time::Duration::from_millis(400).min(rem))
        {
            ...
            if rem_x >= std::time::Duration::from_millis(50) {
                ...
                if let Some(output) =
                    run_cmd_bounded(x_cmd, std::time::Duration::from_millis(400).min(rem_x))
                {
                    if output.status.success() {
                        let text = String::from_utf8_lossy(&output.stdout).trim().to_owned();
