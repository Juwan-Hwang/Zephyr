    ...
    loop {
        match child.try_wait() {
            Ok(Some(status)) => {
                ...
                if let Ok(bytes) = rx.recv_timeout(std::time::Duration::from_millis(20)) {
                    let _ = reader_handle.join();
                    return Some(std::process::Output {
                        status,
                        stdout: bytes,
...
pub fn get_sys_proxy_address_with_deadline(deadline: std::time::Instant) -> Option<String> {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let mut query_gnome = |parts: &mut Vec<String>| {
        ...
        let mut query_kde = |parts: &mut Vec<String>| {
        ...
        let mut query_xfce = |parts: &mut Vec<String>| {
...
pub fn get_sys_proxy_override_bounded(timeout: std::time::Duration) -> Option<String> {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let mut query_gnome_override = || -> Option<String> {
            let rem = timeout.saturating_sub(start.elapsed());
            if rem >= std::time::Duration::from_millis(50) {
                let mut m_cmd = Command::new("gsettings");
                m_cmd.args(["get", "org.gnome.system.proxy", "mode"]);
        ...
        let mut query_kde_override = || -> Option<String> {
            let rem = timeout.saturating_sub(start.elapsed());
            if rem >= std::time::Duration::from_millis(50) {
                if let Some((_, kread_cmd)) =
        ...
        let mut query_xfce_override = || -> Option<String> {
            let rem = timeout.saturating_sub(start.elapsed());
            if rem >= std::time::Duration::from_millis(50)
