    ...
    loop {
        match child.try_wait() {
            Ok(Some(status)) => {
                ...
                let stdout_bytes = if wait_time > std::time::Duration::ZERO {
                    if let Ok(bytes) = rx.recv_timeout(wait_time) {
                        let _ = reader_handle.join();
                        bytes
                    } else {
                        kill_child_and_descendants(&mut child);
                        let rem_grace = timeout.saturating_sub(start.elapsed());
                        let grace_bytes = if rem_grace > std::time::Duration::ZERO {
                            rx.recv_timeout(std::time::Duration::from_millis(50).min(rem_grace))
                                .ok()
                        } else {
                            None
                        };
...
pub fn get_sys_proxy_address_with_deadline(deadline: std::time::Instant) -> Option<String> {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        match desktop {
            ...
            LinuxDesktop::Gnome | LinuxDesktop::Other => {
                query_gnome(&mut all_parts);
                query_kde(&mut all_parts);
                query_xfce(&mut all_parts);
            }
