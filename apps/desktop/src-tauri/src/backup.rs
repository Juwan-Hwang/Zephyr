...
pub async fn import_backup(app: AppHandle) -> Result<String, String> {
    ...
    let staging_result = tokio::task::spawn_blocking(move || -> Result<BackupManifest, String> {
        // Open the archive
    ...
    let manifest = match staging_result {
        Ok(m) => m,
        Err(e) => return Err(e),
    };

    // Determine staging directory path (we need to reconstruct it — but we
    // can't since the timestamp was generated inside the blocking thread).
    // Instead, let's find the staging directory.
    let staging_dir = find_staging_dir(&paths.app_data_dir)?;
    ...
    for (rel_path, dest_abs) in &dest_map {
        if dest_abs.exists() {
            ...
            fs::rename(dest_abs, &backup_path)
                .map_err(|e| format!("Failed to backup {dest_abs:?}: {e}"))?;
        }
    }
    ...
    for (rel_path, dest_abs) in &dest_map {
        ...
        if staged_path.exists() {
            ...
            if let Err(e) = fs::rename(&staged_path, dest_abs) {
...
/// Find the staging directory (there should be exactly one).
fn find_staging_dir(app_data_dir: &Path) -> Result<PathBuf, String> {
    let mut found = Vec::new();
    if let Ok(entries) = fs::read_dir(app_data_dir) {
        for entry in entries.flatten() {
            let name = entry.file_name();
            let name_str = name.to_string_lossy();
            if name_str.starts_with(".backup-staging-") {
                found.push(entry.path());
            }
        }
    }
    match found.len() {
        0 => Err("Staging directory not found".to_owned()),
        1 => Ok(found
            .into_iter()
            .next()
            .ok_or_else(|| "No staging dir".to_owned())?),
        _ => {
            // Clean up all staging dirs and return error
            for dir in &found {
                let _ = fs::remove_dir_all(dir);
            }
            Err("Multiple staging directories found (cleaned up). Please try again.".to_owned())
        }
    }
}
