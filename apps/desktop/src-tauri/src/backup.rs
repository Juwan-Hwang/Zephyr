            ...
            let mut extract_all = || -> Result<(), String> {
                for entry in &manifest.files {
                    ...
                    if entry.path == "settings.json" {
                        ...
                        // Validate that settings.json is a JSON object AND
                        // can be deserialized into the Settings struct.
                        // This catches incompatible types early in staging,
                        // preventing silent data loss during post-import
                        // reload (which would fall back to defaults).
                        match serde_json::from_str::<serde_json::Value>(&content) {
                            Ok(v) if v.is_object() => {
                                if serde_json::from_value::<crate::Settings>(v).is_err() {
                                    return Err(
                                        "Extracted settings.json contains incompatible types"
                                            .to_owned(),
                                    );
                                }
                            }
                            _ => {
                                return Err(
                                    "Extracted settings.json is corrupt or invalid".to_owned()
                                )
                            }
                        }
                    }
