...
pub async fn import_backup(app: AppHandle) -> Result<String, String> {
    ...
    if let Some(settings_state) = app.try_state::<SettingsState>() {
        ...
        let reload_result = tokio::task::spawn_blocking(move || -> Option<crate::Settings> {
            ...
            // to a type mismatch in a future schema), fall back to
            // default Settings so the app doesn't crash.
            let new_settings =
                serde_json::from_value::<crate::Settings>(value).unwrap_or_else(|e| {
                    eprintln!("[Backup] Failed to parse imported settings: {e}");
