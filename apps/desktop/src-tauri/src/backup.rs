...
pub async fn import_backup(app: AppHandle) -> Result<String, String> {
    ...
    if let Some(settings_state) = app.try_state::<SettingsState>() {
        ...
        let reload_result = tokio::task::spawn_blocking(move || -> Option<crate::Settings> {
            ...
            // to a type mismatch in a future schema), return None to
            // avoid overwriting the file on disk with defaults.
            let new_settings = serde_json::from_value::<crate::Settings>(value).ok()?;
            Some(crate::migrate_settings(new_settings, &settings_file))
        })
        .await
        .ok()
        .flatten();

        if let Some(migrated) = reload_result {
