            ...
            let mut extract_all = || -> Result<(), String> {
                for entry in &manifest.files {
                    ...
                    if entry.path == "settings.json" {
                        ...
                        // Validate that settings.json can be deserialized into
                        // the Settings struct. Since Settings has named fields,
                        // serde_json automatically enforces that the input is a
                        // valid JSON object with compatible types.
                        if serde_json::from_str::<crate::Settings>(&content).is_err() {
                            return Err(
                                "Extracted settings.json is corrupt or contains incompatible types"
                                    .to_owned(),
                            );
                        }
                    }
