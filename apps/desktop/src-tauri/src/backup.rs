...
pub async fn export_backup(app: AppHandle) -> Result<String, String> {
    ...
    tokio::task::spawn_blocking(move || -> Result<String, String> {
        ...
        let zip_result = (|| -> Result<(), String> {
            let mut zip_writer = zip::ZipWriter::new(std::io::BufWriter::new(zip_file));
            let zip_options =
            ...
            let mut buffered_file = zip_writer
                .finish()
                .map_err(|e| format!("Failed to finalize zip: {e}"))?;
            buffered_file
                .flush()
                .map_err(|e| format!("Failed to flush zip buffer: {e}"))?;
            buffered_file
                .get_ref()
                .sync_all()
                .map_err(|e| format!("Failed to sync zip file to disk: {e}"))?;
            Ok(())
        })();
...
fn is_safe_archive_path(path: &str) -> bool {
    ...
    // Reject backslashes and colons to prevent NTFS ADS
    ...
    // Component-based validation: reject ".", "..", and empty components
    // (e.g., double slashes "profiles//my-sub.yaml" or "profiles/./my-sub.yaml").
    // This is more robust than substring matching and catches all traversal vectors.
        ...
        Err(rename_err) => {
            ...
            if let Err(e) = fs::copy(src, &temp_path) {
                if let Err(rm_err) = fs::remove_file(&temp_path) {
                    eprintln!("[Backup] Failed to clean up temp file {temp_path:?}: {rm_err}");
                }
                return Err(format!(
...
pub async fn import_backup(app: AppHandle) -> Result<String, String> {
        ...
        tokio::task::spawn_blocking(move || -> Result<(BackupManifest, PathBuf), String> {
            ...
            let mut extract_all = || -> Result<(), String> {
                for entry in &manifest.files {
                    ...
                    // Zip bomb: check compression ratio only for files larger than
                    // 1 MB to avoid false positives on small, highly-compressible
                    // config files (e.g., YAML profiles with many comments).
                    ...
                    if entry.path == "settings.json" {
                        ...
                        // Validate that settings.json is syntactically valid JSON.
                        // We use serde_json::Value (not Settings) here because
                        // older backups may have a different schema version that
                        // wouldn't deserialize into Settings directly. The
                        // migration chain handles schema upgrades during reload.
                        if serde_json::from_str::<serde_json::Value>(&content).is_err() {
                            return Err("Extracted settings.json is not valid JSON".to_owned());
                        }
                    }
                }
                Ok(())
    ...
    let has_settings = manifest.files.iter().any(|f| f.path == "settings.json");
    let paths_clone = paths.clone();
    ...
    let commit_result = tokio::task::spawn_blocking(move || -> Result<(), String> {
        ...
        fs::create_dir_all(&backup_dir).map_err(|e| {
            // Clean up staging dir if rollback dir creation fails to
            // prevent leaked files on disk.
            let _ = fs::remove_dir_all(&staging_dir);
            format!("Failed to create rollback directory: {e}")
        })?;
        ...
        let mut commit_result = || -> Result<(), String> {
            ...
            for (rel_path, dest_abs) in &dest_map {
                ...
                if !staged_path.exists() {
                    return Err(format!("Staged file {rel_path} missing before commit"));
                }
        ...
        if let Err(e) = commit_result() {
            ...
            let _ = fs::remove_dir_all(&staging_dir);
            match restore_rollback(&backup_dir, &paths_clone) {
                Ok(()) => {
                    // Rollback succeeded — safe to delete backup dir
                    let _ = fs::remove_dir_all(&backup_dir);
                }
                Err(rollback_err) => {
                    // Rollback partially failed — preserve backup_dir
                    // so the user can manually recover their files.
                    eprintln!(
                        "[Backup] {e}. Rollback also failed: {rollback_err}. Backup preserved at {backup_dir:?}"
                    );
                    return Err(format!(
                        "{e}. Rollback also failed: {rollback_err}. Original files preserved in {backup_dir:?}"
                    ));
                }
            }
    ...
    //
    // We deserialize to serde_json::Value first, then convert to Settings.
    // This avoids a fragile dependency on direct deserialization: if a
    // future schema migration introduces a breaking change (e.g., changing
    // a field type), direct from_str::<Settings> would fail and bypass
    // the migration chain entirely. Deserializing to Value first allows
    // us to run migrations on the JSON before converting to Settings.
    //
    // All file I/O (read_to_string, migrate_settings write) runs inside
    // spawn_blocking to avoid blocking the async executor.
    if let Some(settings_state) = app.try_state::<SettingsState>() {
        // Only reload SettingsState if the manifest actually contained
        // settings.json. A backup may legitimately omit it (e.g., a fresh
        // install that never persisted settings), and in that case the
        // in-memory settings should remain unchanged.
        if has_settings {
            let settings_arc = std::sync::Arc::clone(&settings_state.0);
            let settings_file = paths.app_data_dir.join("settings.json");

            let reload_result =
                tokio::task::spawn_blocking(move || -> Result<crate::Settings, String> {
                    if !settings_file.exists() {
                        return Err("settings.json does not exist after import".to_owned());
                    }
                    let content = fs::read_to_string(&settings_file)
                        .map_err(|e| format!("Failed to read settings.json: {e}"))?;
                    let value = serde_json::from_str::<serde_json::Value>(&content)
                        .map_err(|e| format!("Failed to parse settings JSON: {e}"))?;
                    // Try converting to Settings; if that fails (e.g., due
                    // to a type mismatch in a future schema), return Err to
                    // avoid overwriting the file on disk with defaults.
                    let new_settings = serde_json::from_value::<crate::Settings>(value)
                        .map_err(|e| format!("Failed to convert JSON to Settings: {e}"))?;
                    Ok(crate::migrate_settings(new_settings, &settings_file))
                })
                .await
                .map_err(|e| format!("Reload task panicked: {e}"))??;

            let mut guard = settings_arc
                .lock()
...
///
/// Returns `Ok(())` if all files were restored successfully, or `Err` if
/// any file failed. The caller should preserve `backup_dir` on `Err` to
/// allow manual recovery.
fn restore_rollback(backup_dir: &Path, paths: &AppPaths) -> Result<(), String> {
        ...
        if let Ok(entries) = fs::read_dir(dir) {
            for entry in entries.flatten() {
                ...
                if path.is_dir() {
                    ...
                    if let Some(dest) = map_rel_to_dest(rel, paths) {
                        files.push((path, dest));
                    } else {
                        eprintln!("[Backup] Unknown path in rollback: {rel:?}");
                    }
                } else {
                    eprintln!("[Backup] Failed to strip prefix for {path:?}");
                }
            }
        }
    }
    ...
    for (src, dest) in &files_to_restore {
        ...
        // Remove the partial replacement at dest before restoring the
        // original. On Windows, fs::rename does not overwrite an existing
        // file, so a partial commit would block the rollback.
        if dest.exists() {
...
fn map_rel_to_dest(rel: &Path, paths: &AppPaths) -> Option<PathBuf> {
    if rel.starts_with("profiles") {
        let file_name = rel.strip_prefix("profiles").unwrap_or(rel);
        Some(paths.profiles_dir.join(file_name))
    } else if rel == Path::new("settings.json") {
        Some(paths.app_data_dir.join("settings.json"))
