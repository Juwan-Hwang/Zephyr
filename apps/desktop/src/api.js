...
function tryProviderCache(missingNames, mergedProxies, now, data) {
  if (!_providerCache || (now - _providerCache.ts) >= _PROVIDER_CACHE_TTL) return null;
  // If previous fetch failed, skip retry within TTL
  if (_providerCache.failed) return { ...data, proxies: mergedProxies };
  // Within TTL: merge whatever cached nodes we have (even partial coverage).
  // This prevents repeated /providers/proxies requests during provider downloads.
