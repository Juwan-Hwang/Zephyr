...
/** @type {number} Maximum log entries forwarded per dedup window */
const MAX_PER_WINDOW = 50;

/** @type {Map<string, number>} message-hash → last-sent timestamp */
const _dedupe = new Map();

/** @type {number} Entries forwarded in the current window */
let _windowCount = 0;
...
export function forwardToBackend(level, source, message) {
    ...
    // — Rate limit: drop if window capacity exhausted —
    // Error-level entries always pass through, even when the window is full,
    // so critical frontend failures are never silently dropped by a burst
    // of lower-severity (debug/info) chatter.
    const isError = level === 'error';
    if (!isError && _windowCount >= MAX_PER_WINDOW) return;
