...
import { pasteToElement, pasteWithFeedback, normalizePastedText, renderPasteButtonHtml } from './clipboard.js';
...
describe('clipboard utils - pasteToElement', () => {
    ...
    describe('race protection', () => {
        ...
        it('aborts paste when selection changed (but value unchanged) while clipboard reading was pending', async () => {
            const mockInput = document.createElement('input');
            mockInput.value = 'initial text';
            mockInput.setSelectionRange(0, 3);
            document.body.appendChild(mockInput);
            ...
            // User moves caret without changing value while clipboard read is pending
            ...
            expect(result).toBe(false);
            expect(mockInput.value).toBe('initial text');
            document.body.removeChild(mockInput);
        });
        ...
        it('suppresses the error callback when a newer paste supersedes a rejected older one', async () => {
            const mockInput = document.createElement('input');
            document.body.appendChild(mockInput);
            const onError = vi.fn();
            ...
            const p1 = pasteToElement(mockInput, true, onError);
            const p2 = pasteToElement(mockInput, true, onError);
    ...
    describe('pasteWithFeedback', () => {
        it('pastes successfully and does not invoke notify', async () => {
            const mockInput = document.createElement('input');
            const notify = vi.fn();
            const translations = { en: { pasteFailed: 'Failed to paste' } };
            ...
            const result = await pasteWithFeedback(mockInput, notify, translations, 'en');
            expect(result).toBe(true);
            expect(mockInput.value).toBe('hello world');
            expect(notify).not.toHaveBeenCalled();
        });

        it('invokes notify with localized failure message on clipboard read error', async () => {
            const mockInput = document.createElement('input');
            mockInput.value = 'original';
            const notify = vi.fn();
            const translations = {
                en: { pasteFailed: 'Failed to paste' },
                zh: { pasteFailed: '粘贴失败' },
            };
            ...
            const result = await pasteWithFeedback(mockInput, notify, translations, 'zh');
            expect(result).toBe(false);
            expect(mockInput.value).toBe('original');
            expect(notify).toHaveBeenCalledWith('粘贴失败', 'warning');
        });

        it('falls back to English when lang is missing from translations', async () => {
            const mockInput = document.createElement('input');
            const notify = vi.fn();
            const translations = { en: { pasteFailed: 'Failed to paste' } };
            ...
            const result = await pasteWithFeedback(mockInput, notify, translations, 'fr');
            expect(result).toBe(false);
            expect(notify).toHaveBeenCalledWith('Failed to paste', 'warning');
        });
    });
});