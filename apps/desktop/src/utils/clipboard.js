 ...
 * @returns {string}
 */
export function renderPasteButtonHtml(id, label = 'Paste', isTextarea = false) {
    const btnClass = isTextarea ? 'btn-textarea-paste' : 'btn-input-paste';
    const escapedLabel = escapeAttr(label);
    return `<button type="button" id="${escapeAttr(id)}" class="${btnClass}" title="${escapedLabel}" aria-label="${escapedLabel}" data-i18n-title="paste" data-i18n-aria-label="paste">${SVG_ICONS.clipboard}</button>`;
}
 ...
 */
function applyPastedText(targetInput, valToInsert) {
...
export async function pasteToElement(targetInput, trim = true) {
    ...
    try {
        ...
        applyPastedText(targetInput, valToInsert);
