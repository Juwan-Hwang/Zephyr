...
/**
 * Checks whether the Clipboard API is available in the current environment.
 * If unavailable, logs a warning and invokes the error callback.
 *
 * @param {((error: Error | string) => void) | null} [onError]
 * @returns {boolean}
 */
function isClipboardReadable(onError) {
    if (typeof navigator !== 'undefined' && typeof navigator.clipboard?.readText === 'function') return true;
...
/**
 * Checks whether the target input's value or selection has changed since a snapshot was taken.
 *
 * @param {HTMLInputElement | HTMLTextAreaElement} targetInput
 * @param {string} snapshotValue
 * @param {number|null} snapshotSelStart
 * @param {number|null} snapshotSelEnd
 * @returns {boolean}
 */
function hasTargetChanged(targetInput, snapshotValue, snapshotSelStart, snapshotSelEnd) {
    const currentSelStart = typeof targetInput.selectionStart === 'number' ? targetInput.selectionStart : null;
    const currentSelEnd = typeof targetInput.selectionEnd === 'number' ? targetInput.selectionEnd : null;
    return targetInput.value !== snapshotValue
        || currentSelStart !== snapshotSelStart
        || currentSelEnd !== snapshotSelEnd;
}
...
export async function pasteToElement(targetInput, trim = true, onError = null) {
    ...
    try {
        if (!isClipboardReadable(onError)) return false;
        ...
        // If the user edited the field or changed the selection while clipboard
        // access was pending, abort to avoid clobbering edits or inserting at a
        // stale caret position.
        if (hasTargetChanged(targetInput, initialValue, selStart, selEnd)) {
            clipboardLogger.warn('Target value or selection changed while reading clipboard; paste aborted to prevent overwriting edits');
            return false;
        }
        ...
        // Suppress error notification if a newer paste has superseded this one
        if (targetWithMeta._pasteVersion === pasteVersion && typeof onError === 'function') {
...
/**
 * Pastes clipboard text into the target and shows a localized warning on failure.
 * Convenience wrapper around pasteToElement that resolves the failure message
 * from the translations map and invokes the provided notify function.
 *
 * @param {HTMLInputElement | HTMLTextAreaElement} targetInput
 * @param {(message: string, level: 'error' | 'info' | 'success' | 'warning') => void} notify
 * @param {Record<string, Record<string, unknown>>} translationsMap
 * @param {string} lang
 * @param {boolean} [trim=true]
 * @returns {Promise<boolean>}
 */
export function pasteWithFeedback(targetInput, notify, translationsMap, lang, trim = true) {
    const failMsg = /** @type {any} */ (translationsMap[lang])?.pasteFailed || /** @type {any} */ (translationsMap.en)?.pasteFailed || 'Failed to paste';
    return pasteToElement(targetInput, trim, () => notify(failMsg, 'warning'));
}
