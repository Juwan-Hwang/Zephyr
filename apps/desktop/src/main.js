 ...
 * Handles prerelease identifiers: `1.18.0` > `1.18.0-alpha.1`.
 * @param {string} remote
 * @param {string} local
 * @returns {boolean}
 */
function isRemoteNewer(remote, local) {
  const parseSemVer = (s) => {
    // Strip build metadata (SemVer 10: build metadata MUST be ignored
    ...
    const prerelease = dashIdx === -1 ? null : cleaned.slice(dashIdx + 1);
    return {
  ...
  // Compare main numeric segments
  const len = Math.max(r.main.length, l.main.length);
  for (let i = 0; i < len; i++) {
    const rv = r.main[i] || 0;
    const lv = l.main[i] || 0;
  ...
  // Main equal: compare prerelease presence
  ...
  // Numeric identifiers compare numerically; numeric < non-numeric; otherwise lexicographic
  const plen = Math.max(r.prerelease.length, l.prerelease.length);
  for (let i = 0; i < plen; i++) {
    const rv = r.prerelease[i];
    const lv = l.prerelease[i];
    if (rv === undefined && lv !== undefined) return false;
    if (rv !== undefined && lv === undefined) return true;
    const rNum = /^\d+$/.test(rv) ? parseInt(rv, 10) : null;
    const lNum = /^\d+$/.test(lv) ? parseInt(lv, 10) : null;
    if (rNum !== null && lNum !== null) {
      if (rNum > lNum) return true;
      if (rNum < lNum) return false;
    } else if (rNum !== null && lNum === null) {
      return false; // numeric < non-numeric (SemVer 11.4.3)
    } else if (rNum === null && lNum !== null) {
      return true; // non-numeric > numeric
    } else {
      if (rv < lv) return false;
      if (rv > lv) return true;
    }
...
async function hasCoreUpdate() {
  try {
    ...
    return isRemoteNewer(remoteVer, normalizeVersion(currentVersion));
  } catch (e) {
