...
async function initApp() {
  ...
  const onOnline = () => {
    try { invoke(COMMANDS.NOTIFY_NETWORK_CHANGE, { source: 'browser_online' }).catch(() => {}); }
    catch { /* Tauri bridge unavailable */ }
  };
  const onOffline = () => {
    try { invoke(COMMANDS.NOTIFY_NETWORK_CHANGE, { source: 'browser_offline' }).catch(() => {}); }
    catch { /* Tauri bridge unavailable */ }
  };
  ...
  listen('network-state-changed', (event) => {
    ...
    registerCleanup(() => { unlisten().catch(() => {}); });
  }).catch((err) => {
