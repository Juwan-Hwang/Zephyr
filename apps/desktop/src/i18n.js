...
export const translations = {
    en: {
        ...
        networkOptimModalDesc: "This will modify system TCP parameters to improve network performance, especially for high-latency connections (RTT > 25ms). You can revert all changes at any time from this settings page.",
        ...
        networkOptimModalInfo: "All changes are fully reversible — click \"Revert\" to restore system defaults at any time. Administrator privileges are required. Linux: changes persist across reboots. macOS: changes reset on reboot. Windows: changes are system-level and persistent.",
    ...
    zh: {
        ...
        networkOptimModalDesc: "此操作将修改系统 TCP 参数以提升网络性能，尤其适用于高延迟连接（RTT > 25ms）。您可以随时在设置页面回退所有修改。",
        ...
        networkOptimModalInfo: "所有修改均可完全回退 — 点击\"回退\"即可随时恢复系统默认值。需要管理员权限。Linux：修改在重启后保留。macOS：修改在重启后失效。Windows：修改为系统级且持久化。",
