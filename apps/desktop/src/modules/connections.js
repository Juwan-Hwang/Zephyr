...
function updateSortIndicators() {
    ...
    for (const cell of header.querySelectorAll('.sortable')) {
        ...
        const originalColors = {
            ...
            dlSpeed: 'color-mix(in srgb, var(--colors-purple-500) 70%, transparent)',
            dlTotal: 'color-mix(in srgb, var(--colors-purple-500) 50%, transparent)',
            ulSpeed: 'color-mix(in srgb, var(--colors-blue-500) 70%, transparent)',
            ulTotal: 'color-mix(in srgb, var(--colors-blue-500) 50%, transparent)',
        };
...
function showConnDetail(conn, mode) {
    ...
    // Network info
    const networkVal = _esc(String(meta.network || meta.interface || '-'));
    const typeVal = _esc(String(meta.type || conn.type || '-'));
    const srcIp = _esc(String(meta.sourceIP || '-'));
    const srcPort = _esc(String(meta.sourcePort || '-'));
