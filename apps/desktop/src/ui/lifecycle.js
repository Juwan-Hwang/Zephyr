...
export async function switchToConfig(configName, customArgs = []) {
  ...
  if (fallbackOccurred || noConfigLoaded) {
    ...
    if (noConfigLoaded) {
      // 没有有效配置，使用了最小配置
      showNotification(
        t.configFallbackMinimal || `Config '${configName}' is invalid. Using minimal config.`,
        'warning'
      );
    } else {
      // 回退到其他配置
      showNotification(
        (t.configFallback || `Config '${configName}' is invalid. Switched to '${actualConfig}'.`)
          .replace('{requested}', configName)
          .replace('{actual}', actualConfig || ''),
        'warning'
      );
    }
