    ...
    formatBtn?.addEventListener('click', async () => {
        ...
        modal.innerHTML = `
            <div class="glass-card w-[360px] p-6 space-y-4">
                ...
                <div class="space-y-1">
                    ${formats.map(f => `
                        <button class="copy-env-format-option w-full flex items-center justify-between px-3 py-2.5 rounded-[var(--zephyr-radius-surface)] text-left transition-colors hover:bg-[var(--zephyr-bg-muted)] ${f.key === currentFormat ? 'bg-[var(--zephyr-bg-muted)]' : ''}" data-format="${f.key}">
