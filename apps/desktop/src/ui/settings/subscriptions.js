        ...
        sortedConfigs.forEach((/** @type {any} */ configInfo) => {
            ...
            if (configInfo.sub_info) {
                ...
                if (total > 0) {
                    ...
                    barFill.className = `h-full rounded-full transition-[width] duration-[var(--zephyr-time-page)] ${percentage > 90 ? 'bg-danger' : 'bg-accent'}`;
                    barFill.style.width = `${percentage}%`;

                    barBg.setAttribute('role', 'progressbar');
                    barBg.setAttribute('aria-valuenow', String(Math.round(percentage)));
