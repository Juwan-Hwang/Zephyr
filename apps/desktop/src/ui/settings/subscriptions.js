...
export function initSubscriptionSettings({
    ...
    const showSubscriptionContextMenu = async (e, configInfo) => {
        ...
        editItem.innerHTML = `<svg class="w-3.5 h-3.5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/><path d="m15 5 4 4"/></svg><span>${escapeHtml(t.edit || 'Edit')}</span>`; // nosemgrep: js-innerhtml-assignment — verified safe, see eslint-disable above
        ...
        try {
            ...
            if (files.length === 0) {
                ...
                for (const [groupName, groupFiles] of grouped) {
                    ...
                    // eslint-disable-next-line no-unsanitized/property -- values escaped via escapeHtml()
                    groupHeader.innerHTML = `<svg class="w-3 h-3 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/></svg>${escapeHtml(groupName)}`; // nosemgrep: js-innerhtml-assignment — verified safe, see eslint-disable above
