...
function buildOverrideCard(item) {
    ...
    card.innerHTML = `
        ...
        <div class="flex items-center gap-2 shrink-0">
            ...
            <button class="override-up-btn opacity-0 group-hover:opacity-100 p-1.5 rounded-[var(--zephyr-radius-control)] text-[var(--text-muted)] hover:text-accent hover:bg-accent/10 transition-[opacity,color] ${isFirst(item.id) ? 'invisible' : ''}" title="${escapeAttr(t('overrideMoveUp') || '上移')}" data-id="${item.id}">
                ${SVG_ICONS.arrowUp}
            </button>
