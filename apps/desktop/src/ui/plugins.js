...
async function importFromUrl() {
    ...
    const ext = trimmedUrl.endsWith('.yaml') || trimmedUrl.endsWith('.yml')
        ? 'prism.yaml'
        : 'js';

    try {
        await overrideCreate(
            trimmedUrl.split('/').pop()?.split('?')[0] || 'Remote Override',
            ext,
            'remote',
            trimmedUrl,
        );
...
async function loadOverrides() {
    ...
    if (!pluginList._dragBound) {
        ...
        document.addEventListener('mousemove', (e) => {
            ...
            // First move — create floating clone
            if (!_dragState.moved) {
                _dragState.moved = true;
                const rect = _dragState.el.getBoundingClientRect();
                _dragState.elHeight = rect.height;
                ...
                // currentIndex in the querySelectorAll card list
                _dragState.currentIndex = getOverrideCards(pluginList).indexOf(_dragState.el);
            }

            // Move clone with mouse
            const cloneY = e.clientY - _dragState.offsetY;
            _dragState.clone.style.top = cloneY + 'px';

            // Calculate insert position: the index where el should land
            // in the FINAL array after removal from currentIndex.
            // Use a simple approach: count how many non-dragged cards the
            // cursor has passed, then add 1 for the dragged card itself
            // if it was above.
            const cards = getOverrideCards(pluginList);
            const listRect = pluginList.getBoundingClientRect();
            const relativeY = e.clientY - listRect.top + pluginList.scrollTop;

            let insertAfter = -1; // -1 means "insert at very beginning"
            for (let i = 0; i < cards.length; i++) {
                if (cards[i] === _dragState.el) continue;
                const cardRect = cards[i].getBoundingClientRect();
                const cardMid = cardRect.top + cardRect.height / 2 - listRect.top + pluginList.scrollTop;
            ...
            // Animate cards to make room for the placeholder.
            // insertAfter is the index (in original DOM order) of the last
            // card the cursor has passed. Cards between currentIndex and
            // insertAfter need to shift.
            cards.forEach((card, i) => {
                if (card === _dragState.el) return;
                let offset = 0;
                if (_dragState.currentIndex < i && i <= insertAfter) {
                    // Dragging down: cards between original position and
                    // target slide up to make room.
                    offset = -(_dragState.elHeight + gap);
                } else if (insertAfter < i && i < _dragState.currentIndex) {
                    // Dragging up: cards between target and original
                    // position slide down to make room.
                    offset = _dragState.elHeight + gap;
                }
        ...
        document.addEventListener('mouseup', async (e) => {
            if (!_dragState) return;
            const { el, moved, clone, targetIndex, currentIndex } = _dragState;
            _dragState = null;
