...
function buildProxyWrappers(container, proxies, data, current, mainGroup) {
    ...
    const createCard = (/** @type {HTMLElement} */ wrapper) => {
        ...
        nameContainer.className = `flex-1 text-sm font-semibold text-[var(--text-primary)] tracking-tight transition-colors duration-[var(--zephyr-time-standard)] ${isScrolling ? 'scrolling-text-container' : 'overflow-hidden'}`;
