...
/**
 * Group name for which provider polling was exhausted (null = not exhausted).
 * Prevents re-entering the poll loop for the same group after max retries.
 */
let _providerPollExhaustedGroup = null;
...
function renderProviderPollExhausted(container, tObj) {
    ...
    retryBtn.addEventListener('click', () => {
        _providerPollExhaustedGroup = null;
        renderProxies();
    });
...
function startProviderPoll(preferredGroupName, attempt = 0) {
    ...
    if (attempt >= PROVIDER_POLL_MAX) {
        ...
        _providerPollExhaustedGroup = preferredGroupName;
        renderProxies();
        return;
    }
...
export async function renderProxies() {
    ...
    if (proxyGroupsResult.providerLoading) {
        ...
        if (_providerPollExhaustedGroup === preferredGroupName) {
