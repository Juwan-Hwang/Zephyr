...
function inferEffectiveGroup(runConfig, proxyMap) {
    ...
    for (let i = runConfig.rules.length - 1; i >= 0; i--) {
        ...
        // Use indexOf-based parsing to handle group names containing commas.
        const firstComma = rule.indexOf(',');
        if (firstComma === -1) continue;
        const secondComma = rule.indexOf(',', firstComma + 1);
        let candidate = (secondComma === -1
            ? rule.substring(firstComma + 1)
            : rule.substring(firstComma + 1, secondComma)).trim();

        // Strip surrounding quotes (single or double) from the group name
...
export async function fetchProxyGroups(options = {}) {
    ...
    if (config?.mode?.toLowerCase() === 'global' && proxyMap['GLOBAL']) {
        uiGroupName = 'GLOBAL';
    }
    ...
    return {
        ...
        config,
        groups,
        mainGroup: uiPrimaryGroupName,  // Backward compat — null signals "not ready"
        proxies,
        current,
