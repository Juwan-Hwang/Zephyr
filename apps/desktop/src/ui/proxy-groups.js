 ...
 * @property {boolean} providerLoading - True when uiGroup uses include-all and all[] has no real proxy nodes (empty or special-only)
...
export async function fetchProxyGroups(options = {}) {
    ...
    const isIncludeAllGroup = uiGroupName
        ...
        : false;
    const hasRealProxies = proxies.some(name => !SPECIAL_GROUPS.has(name.toUpperCase()));
    ...
    return {
        ...
        // and its all[] has no real proxy nodes (empty or only special
        // entries like COMPATIBLE/DIRECT/REJECT/PASS — nodes still loading)
        providerLoading,
