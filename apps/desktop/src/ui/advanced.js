...
function renderSimpleArrayEditor(container, arr, fullKey) {
    ...
    textarea.className = "w-full min-h-[60px] bg-[var(--zephyr-bg-input)] border border-[var(--zephyr-border-subtle)] rounded-[var(--zephyr-radius-control)] px-3 py-2 text-2xs text-[var(--text-secondary)] focus:outline-none focus:border-accent/50 transition-colors font-mono resize-y";
...
function renderConfigItem(key, value, fullKey) {
    ...
    if (typeof value === 'boolean') {
        ...
        setBtn.onclick = (e) => {
            ...
            const input = document.createElement('input');
            input.type = "text";
            input.className = "w-full max-w-[120px] bg-[var(--zephyr-bg-input)] border border-accent/50 rounded-[var(--zephyr-radius-control)] px-2 py-1 text-xs text-[var(--text-secondary)] focus:outline-none font-mono";
