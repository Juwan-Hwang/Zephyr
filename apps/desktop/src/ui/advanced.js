...
async function handleConfigUpdate(path, value) {
    ...
    try {
        ...
        if (!persisted) {
            ...
            const noConfigMsg = typeof t.notifNoActiveConfig === 'string' ? t.notifNoActiveConfig : 'No active configuration or YAML parser unavailable';
            showNotification(`${errorPrefix}: ${noConfigMsg}`, 'error');
