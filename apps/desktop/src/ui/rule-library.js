...
function buildGroupSection(group, files, fileMap, isUngrouped = false) {
    ...
    header.addEventListener('click', () => {
        ...
        const arrow = /** @type {SVGElement|null} */ (header.querySelector('.collapse-arrow'));
    ...
    activeRulesVs = createVirtualScroll({
        ...
        renderItem(idx, fragment) {
            ...
            if (item.kind === 'group-header') {
                ...
                `;
                header.addEventListener('click', () => {
                    const activeLineIdx = document.activeElement?.getAttribute('data-line-idx');
                    ...
                    renderActiveRules();
                    if (activeLineIdx) {
                        const newEl = container?.querySelector('[data-line-idx="' + activeLineIdx + '"]');
                        (/** @type {HTMLElement} */ (newEl))?.focus();
                    }
                });
...
async function handleImportRules() {
        ...
        <div class="space-y-4">
            ...
            <div id="rl-import-paste" class="space-y-2">
                <div class="flex justify-end">
                    <button type="button" id="rl-import-paste-btn" class="btn-textarea-paste" title="${escapeAttr(t.paste || 'Paste')}" aria-label="${escapeAttr(t.paste || 'Paste')}" data-i18n-title="paste" data-i18n-aria-label="paste">
                        ${SVG_ICONS.clipboard}
                        <span>${escapeHtml(t.ruleLibraryImportPaste || t.paste || 'Paste')}</span>
                    </button>
                </div>
                <textarea id="rl-import-text" aria-label="${escapeAttr(t.ruleLibraryImportPaste || 'Rule Text')}" class="form-control form-control-mono w-full h-32 rounded-xl p-3 text-xs resize-none custom-scrollbar" placeholder="${escapeHtml(t.ruleLibraryImportPaste || '')}" spellcheck="false"></textarea>
            </div>
            <div id="rl-import-file" class="hidden space-y-3">
                <input id="rl-import-file-path" aria-label="${escapeAttr(t.ruleLibraryImportFile || 'File Path')}" type="text" class="form-control form-control-lg" placeholder="${escapeHtml(t.ruleLibraryImportFile || '')}" />
            </div>
            <div id="rl-import-url" class="hidden space-y-3">
                <div class="input-paste-wrapper">
                    ...
                    <button type="button" id="rl-import-url-paste-btn" class="btn-input-paste" title="${escapeAttr(t.paste || 'Paste')}" aria-label="${escapeAttr(t.paste || 'Paste')}" data-i18n-title="paste" data-i18n-aria-label="paste">${SVG_ICONS.clipboard}</button>
                </div>
            </div>
            <div class="space-y-2">
                <label for="rl-import-name" class="text-xs text-[var(--text-muted)]">${escapeHtml(t.ruleLibraryGroupName || 'Name')}</label>
    ...
    showModal(
        ...
        (contentArea, close) => {
            ...
            contentArea.querySelector('#rl-import-paste-btn')?.addEventListener('click', (ev) => {
                ev.preventDefault();
                const textarea = /** @type {HTMLTextAreaElement | null} */ (contentArea.querySelector('#rl-import-text'));
                if (textarea) pasteToElement(textarea, false);
            });
            contentArea.querySelector('#rl-import-url-paste-btn')?.addEventListener('click', (ev) => {
                ev.preventDefault();
                const urlInput = /** @type {HTMLInputElement | null} */ (contentArea.querySelector('#rl-import-url-input'));
                if (urlInput) pasteToElement(urlInput);
            });
    ...
    if (profiles.length === 1) {
        ...
        const profileOptionsHtml = profiles
            .map((/** @type {any} */ p, /** @type {number} */ i) => {
                ...
                return `<button type="button" class="w-full text-start px-4 py-3 text-sm text-[var(--text-secondary)] hover:bg-accent/10 hover:text-accent transition-colors rounded-lg flex items-center justify-between group/prof" data-profile-id="${escapeAttr(String(pId))}">
