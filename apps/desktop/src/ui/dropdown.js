...
export function initCustomDropdown({ wrapId, triggerId, menuId, labelId, selectId, onChange, optionAttr = 'data-value' }) {
    ...
    menu.addEventListener('wheel', (e) => {
        ...
        if (canScroll) {
            ...
            e.stopPropagation();
        } else if (scrollAncestor && scrollAncestor.scrollHeight > scrollAncestor.clientHeight) {
            ...
            // Guard: only intercept when the ancestor is actually scrollable
            // (overflow-y set but content doesn't overflow = no-op, let
            // the event propagate naturally to avoid swallowing wheel input).
            e.preventDefault();
