    ...
    for (let i = 0; i < currentConfigRules.length; i++) {
        ...
        item.innerHTML = `
            ...
            <div class="flex items-center gap-2">
                ...
                <button class="btn-move-top-rule opacity-0 group-hover:opacity-100 p-1.5 rounded-[var(--zephyr-radius-control)] text-[var(--text-muted)] hover:text-accent hover:bg-accent/10 transition-[opacity,color]" title="${escapeAttr(/** @type {any} */ (translations)[currentLang].moveToTop || 'Move to Top')}">
                    ${SVG_ICONS.arrowUp}
                </button>
                <button class="btn-move-bottom-rule opacity-0 group-hover:opacity-100 p-1.5 rounded-[var(--zephyr-radius-control)] text-[var(--text-muted)] hover:text-accent hover:bg-accent/10 transition-[opacity,color]" title="${escapeAttr(/** @type {any} */ (translations)[currentLang].moveToBottom || 'Move to Bottom')}">
                    ${SVG_ICONS.arrowDown}
                </button>
