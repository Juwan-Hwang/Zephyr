...
function renderNumbers() {
    ...
    if (state.downPeak > 0) {
        ...
        if (pk) pk.textContent = fmtSpeedStr(state.downPeak);
    }
    if (state.upPeak > 0) {
        const pk = $('dz-up-peak');
        if (pk) pk.textContent = fmtSpeedStr(state.upPeak);
    }
    ...
    if (avgEl) avgEl.textContent = fmtSpeedStr(avg) + ' ↓';
...
export function initConsoleHome() {
    ...
    const _configUpdatedUnsub = Bus.on(Events.CONFIG_UPDATED, () => {
        ...
        // Reset lifetime totals and peaks to match the fresh core session
        state.totalDown = 0;
        state.totalUp = 0;
        state.downPeak = 0;
        state.upPeak = 0;
