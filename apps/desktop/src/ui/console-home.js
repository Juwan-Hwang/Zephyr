...
function renderNextUpdate(cfg) {
    ...
    if (interval > 0 && cfg.last_updated) {
        ...
        const hm = String(nextDate.getHours()).padStart(2, '0') + ':' + String(nextDate.getMinutes()).padStart(2, '0');
        nextEl.textContent = isSameDay ? hm
            : `${nextDate.getMonth() + 1}/${nextDate.getDate()} ${hm}`;
    } else {
        nextEl.textContent = '—';
    }
}
