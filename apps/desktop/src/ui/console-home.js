...
function renderNumbers() {
    ...
    const elDown = $('dz-down-val'), elDownU = $('dz-down-unit');
    const elUp = $('dz-up-val'), elUpU = $('dz-up-unit');
    if (elDown) elDown.textContent = d.val;
    if (elDownU) elDownU.textContent = d.unit;
    if (elUp) elUp.textContent = u.val;
    if (elUpU) elUpU.textContent = u.unit;

    if (state.down > state.downPeak) {
        state.downPeak = state.down;
        const pk = $('dz-down-peak');
        if (pk) pk.textContent = fmtSpeed(state.downPeak).val + ' ' + fmtSpeed(state.downPeak).unit;
    }
    if (state.up > state.upPeak) {
        state.upPeak = state.up;
        const pk = $('dz-up-peak');
        if (pk) pk.textContent = fmtSpeed(state.upPeak).val + ' ' + fmtSpeed(state.upPeak).unit;
    }

    const avg = trafficHistory.length > 0
        ? trafficHistory.reduce((s, p) => s + p.down, 0) / trafficHistory.length
        : 0;
    const avgEl = $('dz-avg-down');
    if (avgEl) avgEl.textContent = fmtSpeed(avg).val + ' ' + fmtSpeed(avg).unit + ' ↓';

    // KPI: Today's traffic (session accumulated)
    const totalVal = $('dz-total-val');
    if (totalVal) totalVal.textContent = ((state.totalDown + state.totalUp) / 1073741824).toFixed(2);
    const totalDownEl = $('dz-total-down');
    if (totalDownEl) totalDownEl.textContent = (state.totalDown / 1073741824).toFixed(2);
    const totalUpEl = $('dz-total-up');
    if (totalUpEl) totalUpEl.textContent = (state.totalUp / 1073741824).toFixed(2);

    // Chart meta: Session traffic
    const sessDown = $('dz-sess-down');
    if (sessDown) sessDown.textContent = fmtBytes(state.sessDown);
    const sessUp = $('dz-sess-up');
    if (sessUp) sessUp.textContent = fmtBytes(state.sessUp);
}

function renderUptime() {
    if (coreUptimeSec === null) return;
...
function processConnections(rawConns) {
    const now = Date.now();
    const rows = rawConns.slice(0, 20).map((c) => {
        const md = c.metadata || {};
        const proc = md.process || md.processPath || t('unknown');
        const host = md.host ? md.host + ':' + (md.destinationPort || '') : (md.destinationIP || 'unknown');
...
async function refreshNodeData() {
    ...
    let success = false;
    try {
        // Invalidate caches so we always fetch fresh data after a subscription switch.
        // fetchProxyGroups calls getProxies() + getRunConfigCached() internally;
        // without invalidation, stale cached data from the previous subscription
        // would be returned.
        ...
        // Fetch subscription configs for picker tabs — same data source as
        // the subscriptions page: LIST_CONFIGS filtered by url_display.
        try {
            const settings = await getSettingsCached();
            const currentConfig = settings?.last_config || '';
            const allConfigs = await invoke(COMMANDS.LIST_CONFIGS);
            subConfigs = (allConfigs || [])
        ...
        // mihomo's /proxies API omits server/port (sensitive), and provider
        // merge is unreliable. The config file always has them.
        ...
        // Build the flat node list for the picker — all nodes in the
        // active proxy group, shown regardless of which subscription they
        // came from. Subscription switching is handled by the tabs above.
        const activeGroupAll = proxyMap[uiGroupName]?.all || [];
        pickerNodes = activeGroupAll.slice(0, 80).map((/** @type {string} */ name) => ({
            name,
        ...
        // Update node card — pass both proxyMap (for type/history) and hostMap (for server/port)
...
async function refreshSubscriptionData() {
    ...
    try {
        ...
        const subConfigs = (configs || []).filter((/** @type {any} */ c) => c.url_display);
        // Show the currently active subscription, not just the first one
        _subConfig = subConfigs.find((/** @type {any} */ c) => c.name === currentConfig)
            || subConfigs[0]
            || null;
...
function renderSubscriptionCard() {
    ...
    // Parse sub_info: "upload=...; download=...; total=..."
    const subInfo = _subConfig.sub_info;
    let used = 0, total = 0;
    if (subInfo) {
        const parts = subInfo.split(';').map((/** @type {string} */ s) => s.trim());
        let upload = 0, download = 0;
        parts.forEach((/** @type {string} */ p) => {
    ...
    if (updatedEl) {
        ...
        updatedEl.textContent = nodeCount > 0 ? `${timeStr} · ${t('consoleSubNodes', { count: String(nodeCount) })}` : timeStr;
    }

    // Expiry time — parse from sub_info expire= field or subscription config
    const expiryEl = $('dz-sub-expiry');
    if (expiryEl) {
        let expireTs = 0;
        const subInfo = _subConfig.sub_info;
        if (subInfo) {
            ...
            for (const p of parts) {
                if (p.startsWith('expire=')) {
                    expireTs = parseInt(p.split('=')[1], 10) || 0;
                    break;
                }
            }
        }
        if (expireTs > 0) {
            ...
            expiryEl.textContent = diffDays > 0 ? `${dateStr} · ${t('consoleSubDaysLeft', { d: String(diffDays) })}` : dateStr;
        } else {
    ...
    if (nextEl) {
        const interval = _subConfig.auto_update_interval || 0;
        if (interval > 0 && _subConfig.last_updated) {
            const nextTs = (_subConfig.last_updated + interval) * 1000;
            const nextDate = new Date(nextTs);
            nextEl.textContent = String(nextDate.getHours()).padStart(2, '0') + ':' + String(nextDate.getMinutes()).padStart(2, '0');
        } else {
            nextEl.textContent = '—';
        }
    }
...
async function updateSubscription() {
    ...
    try {
        ...
        if (Array.isArray(results) && results.length > 0 && results[0].success) {
            ...
            consoleLogger.warn('Subscription update failed');
        }
    } catch (e) {
...
function pushLog(entry) {
    ...
    while (el.children.length > MAX_LOG_LINES) {
        ...
        if (last) el.removeChild(last); else break;
    }
...
function loadInitialLogs() {
    ...
    const recent = events.slice(-MAX_LOG_LINES).reverse();
...
<div class="dz-content custom-scrollbar">
    ...
    <div class="dz-kpis">
        ...
        <div class="glass-card dz-kpi">
            <div class="dz-kpi-top">
                <span class="dz-kpi-label" data-i18n="consoleTodayTraffic">今日流量</span>
    ...
    <div class="dz-grid-main">
        ...
        <div class="dz-side">
            <div class="dz-node-wrap">
                <div class="glass-card dz-card-pad">
                    <div class="dz-card-head" style="margin-bottom:10px;">
                        ...
                        <div style="display:flex;align-items:center;gap:8px;">
                            ...
                            <button class="dz-switch-node-btn" id="dz-node-picker-btn" aria-haspopup="listbox" aria-expanded="false">
                    ...
                    <div class="dz-node-sub">
                        <span style="font-family:var(--font-mono)" id="dz-node-host" title="点击复制"></span>
                    </div>
                ...
                <div class="dz-node-picker dz-hidden" id="dz-node-picker" role="listbox" aria-label="切换节点">
                    <div class="dz-picker-subs" id="dz-picker-subs"></div>
                    <div class="dz-picker-list custom-scrollbar" id="dz-picker-list"></div>
                </div>
            </div>
                ...
                <div class="dz-quick-rows">
                    <div class="dz-quick-row">
                        <span class="dz-quick-row-label">
                            ...
                            <span data-i18n="consoleSysProxy">系统代理</span>
                        </span>
                        <label class="ios-switch switch-lg"><input type="checkbox" id="dz-sys-toggle"><span class="switch-slider"></span></label>
                    </div>
                    <div class="dz-quick-row">
                        <span class="dz-quick-row-label">
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="3" width="20" height="14" rx="2" ry="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg>
                            <span data-i18n="consoleTunAdapter">TUN 虚拟网卡</span>
                        </span>
                        <label class="ios-switch switch-lg"><input type="checkbox" id="dz-tun-toggle"><span class="switch-slider"></span></label>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="dz-grid-bottom">
        <div class="glass-card dz-card-pad" style="display:flex;flex-direction:column;min-height:0;">
            <div class="dz-card-head">
                ...
                <button class="dz-sort-btn" id="dz-sort-btn" title="点击切换排序方式">
...
export function initConsoleHome() {
    ...
    // Set aria-labels (not handled by applyTranslations)
    const nodePickerInit = $('dz-node-picker');
    if (nodePickerInit) nodePickerInit.setAttribute('aria-label', t('consoleNodePicker'));
...
hostCopyEl?.addEventListener('click', () => {
    const text = hostCopyEl.textContent || '';
    if (!text) return;
    const done = () => {
        const orig = hostCopyEl.style.color;
        hostCopyEl.style.color = 'var(--color-success)';
    ...
    _trafficUnsub = Bus.on(Events.TRAFFIC_UPDATE, (/** @type {any} */ data) => {
        ...
        state.up = raw.up || 0;
        state.sessDown += state.down;
        state.sessUp += state.up;
        state.totalDown += state.down;
        state.totalUp += state.up;
        trafficHistory.push({ up: state.up, down: state.down, time: Date.now() });
    ...
    // ── Log stream ──
    loadInitialLogs();
    const _logCb = (/** @type {{ type: string, message: string, timestamp: string, source?: string }} */ entry) => pushLog(entry);
    ...
    // ── Core restart / config update: refresh all console data ──
    // Only listen to CONFIG_UPDATED (emitted after full postRestartRecovery:
    // overrides applied, proxy selection restored, caches invalidated).
    // CORE_RESTARTED fires too early — mihomo just started, overrides not yet
    // applied, caches may get repopulated with pre-override data.
    ...
    // ── Connection polling ──
    connPollTimer = setInterval(pollConnections, 2000);
    ...
    subUpdateBtn?.addEventListener('click', async () => {
        ...
        // SVG + i18n string — no user input, safe for innerHTML
        // eslint-disable-next-line no-unsanitized/property
        btn.innerHTML = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" class="animate-spin" style="width:12px;height:12px;"><path d="M21 12a9 9 0 1 1-9-9c2.52 0 4.93 1 6.74 2.74L21 8"/><path d="M21 3v5h-5"/></svg><span>${t('consoleSubUpdating')}</span>`;
        btn.disabled = true;
        try {
    ...
    _cleanups.push(Bus.on(Events.I18N_APPLIED, () => {
        ...
        // Aria-labels
        const nodePickerEl = $('dz-node-picker');
        if (nodePickerEl) nodePickerEl.setAttribute('aria-label', t('consoleNodePicker'));
 ...
 * The backend records SystemTime::now() when the process is spawned and
...
async function fetchCoreUptime() {
    try {
        ...
        if (typeof uptime === 'number') {
            coreUptimeSec = uptime;
            renderUptime();
        }
    } catch {
        // Core not started yet — will be retried on activateConsole / CONFIG_UPDATED
    }
}
