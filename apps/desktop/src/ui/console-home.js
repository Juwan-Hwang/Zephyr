...
function drawSeries(ctx, w, h, data, key, max, rgb, fillTop) {
    ...
    // Fill path: bottom-left → data curve → bottom-right → close.
    buildPath('lineTo');
    ctx.lineTo(w, h);
    ctx.closePath();
...
function renderPicker() {
    ...
    if (!listEl.dataset.delegated) {
        listEl.addEventListener('click', (/** @type {Event} */ ev) => {
        ...
        listEl.addEventListener('keydown', (/** @type {KeyboardEvent} */ ev) => {
            const target = /** @type {HTMLElement|null} */ (ev.target);
            const row = /** @type {HTMLElement|null} */ (target ? target.closest('.dz-picker-node') : null);
...
function _buildConsoleHome(container) {
    ...
    if (sysToggle) {
        ...
        sysToggle.addEventListener('change', async () => {
            ...
            try {
                ...
                showNotification(errorPrefix + ': System proxy toggle failed', 'error');
    ...
    /** @type {number|null} */ let lastRawDown = null;
    /** @type {number|null} */ let lastRawUp = null;
    _cleanups.push(Bus.on(Events.TRAFFIC_UPDATE, (/** @type {any} */ data) => {
        const raw = data?.raw;
        if (!raw) return;
        const rawDown = raw.down || 0;
        const rawUp = raw.up || 0;

        // Calculate rates from cumulative byte counters.
        // mihomo /traffic returns cumulative bytes, not per-second rates.
        const now = Date.now();
        const dtSecRaw = lastRawDown !== null ? Math.max(0, (now - lastTrafficTsMsTotal) / 1000) : 0;
        const downRate = (lastRawDown !== null && dtSecRaw > 0) ? Math.max(0, (rawDown - lastRawDown) / dtSecRaw) : 0;
        const upRate = (lastRawUp !== null && dtSecRaw > 0) ? Math.max(0, (rawUp - lastRawUp) / dtSecRaw) : 0;
        state.down = downRate;
        state.up = upRate;
        lastRawDown = rawDown;
        lastRawUp = rawUp;

        // Integrate rates into totals (state.up/down are bytes/sec)
        ...
        if (lastTrafficTsMsTotal && dtSecRaw > 0) {
            const dtSec = Math.max(0, Math.min(5, dtSecRaw));
            state.totalDown += downRate * dtSec;
            state.totalUp += upRate * dtSec;
        }
        ...
        if (isActive) {
            if (lastTrafficTsMsSession) {
                ...
                state.sessDown += downRate * dtSec;
                state.sessUp += upRate * dtSec;
            }
