...
export async function restoreProxySelection(profileName) {
    try {
        ...
        if (proxyGroupsResult.providerLoading) {
            ...
            // If providers never finished loading (timeout), abort with a
            // warning rather than silently proceeding with empty all[] arrays.
            if (!proxyGroupsResult || proxyGroupsResult.providerLoading) {
