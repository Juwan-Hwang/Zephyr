...
async function waitForProvidersLoaded(preferredGroupName, savedNode) {
    ...
    for (let i = 0; i < PROVIDER_WAIT_MAX; i++) {
        ...
        if (!result.providerLoading) {
            // providerLoading cleared — but if we're looking for a specific
            // node that hasn't appeared yet and the config has proxy-providers,
            // keep polling (the node's provider may still be downloading
            // out-of-order, even though the UI group already has some nodes).
            if (!(savedNode && result.hasProxyProviders
                && !nodeInWritableGroup(result, savedNode))) {
