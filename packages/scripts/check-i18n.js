...
class ObjectLiteralParser {
    ...
    parseObject() {
        ...
        while (this.content[this.pos] !== '}') {
            ...
            this.pos++;
            if (Object.hasOwn(obj, key)) this.duplicateKeys.push(key);
...
function loadTranslations(content) {
    ...
    if (content[pos] !== '{') throw new Error(`Expected '{' after "export const translations =" at position ${pos}`);

    const parser = new ObjectLiteralParser(content, pos);
