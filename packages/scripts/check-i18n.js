...
function getRequiredPluralCategories(locale) {
    ...
    const cats = new Set();
    // Test a wide range of integer counts
    for (let n = 0; n <= 200; n++) cats.add(rules.select(n));
    // Test fractional values — these may return 'other' for some locales
    cats.add(rules.select(0.5));
    cats.add(rules.select(1.5));
    cats.add(rules.select(2.5));
    // 'other' is always a valid runtime fallback (pluralObj[category] || pluralObj.other)
    cats.add('other');
...
function main() {
    ...
    for (const locale of targetLocales) {
        ...
        for (const key of enKeys) {
            if (!Object.hasOwn(localeObj, key)) {
                ...
                if (!isTypeCompatible(enVal, val)) {
                    ...
                    // Detect stale nested sub-keys (in target but
                    // not in en's plural object) and unused sub-keys
                    // (in en but not required by this locale's plural rules)
                    for (const sub of Object.keys(val)) {
                        if (!Object.hasOwn(enVal, sub)) {
                            extra.push(`${key}.${sub}`);
                        } else if (!requiredCats.has(sub)) {
                            unused.push(`${key}.${sub}`);
                        }
                    }
                }
        ...
        const localeIssues = missing.length + empty.length + typeMismatch.length + extra.length + unused.length;
        ...
        if (localeIssues === 0) {
            ...
            if (unused.length > 0) {
                console.log(`  Unused plural forms (${unused.length}, present in en but not selected by ${locale} plural rules):`);
