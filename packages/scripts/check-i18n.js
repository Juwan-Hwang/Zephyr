...
function loadTranslations(content) {
    ...
    const declMatch = /export\s+const\s+translations\s*=/g.exec(content);
    if (!declMatch) {
    ...
    // Skip whitespace and comments between `=` and `{` — supports
    // `= /* comment */ {` and `= // comment\n {` in addition to `= {`.
    ...
    if (content[pos] !== '{') {
        throw new Error(`Expected '{' after "export const translations =" at position ${pos}`);
    }
    ...
    function parseEscape() {
        ...
        if (next === undefined) throw new Error(`Unterminated escape sequence at position ${pos}`);
        ...
        for (const key of enKeys) {
            if (!Object.hasOwn(localeObj, key)) {
                ...
                if (!isTypeCompatible(enVal, val)) {
                    ...
                    // verify each plural category exists and is non-empty.
